...
Expect a few minutes. The script clones Panda3D at a pinned revision, gets the
dependencies that revision needs, builds a wheel, and then installs that wheel
into a throwaway virtual environment to check it: one missing ODE, OpenAL,
FFmpeg or JPEG support fails here rather than halfway through a release.
...
macOS builds them from upstream's thirdparty scripts. The tarball panda3d.org
ships is cut for 1.10.16 and fails twice over on a current toolchain: its
archive members are not 8-byte aligned, which the linker rejects, and it
carries a pre-C++17 OpenEXR that master will not compile.

Linux takes them from apt, and Windows downloads the prebuilt tools zip that
upstream's own CI builds master against.
