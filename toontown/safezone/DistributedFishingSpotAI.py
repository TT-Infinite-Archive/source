...
class DistributedFishingSpotAI(DistributedObjectAI):
    ...
    def removeFromPierWithAnim(self):
        ...
        taskMgr.doMethodLater(1.5, DistributedFishingSpotAI.removeFromPier, 'remove%d' % self.doId, [self])
    ...
    def rewardIfValid(self, target):
        ...
        if self.air.wantGuilds and self.air.wantGuildQuests:
            if catch[0] in (FishGlobals.FishItem, FishGlobals.FishItemNewEntry, FishGlobals.FishItemNewRecord):
                fish = FishBase(catch[1], catch[2], catch[3])
                self.air.guildManager.handleFishCaptured(self.avId, fish)
