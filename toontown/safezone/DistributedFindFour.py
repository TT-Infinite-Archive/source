...
class DistributedFindFour(DistributedNode.DistributedNode):
    ...
    def moveCameraForGame(self):
        ...
        if self.seatPos <= 2:
            ...
            position = position + Vec3(0, -8, 11.8)
            interval = LerpPosHprInterval(base.camera, 2, position, Vec3(0, -38, 0), base.camera.getPos(), base.camera.getHpr())
        else:
            position = self.table.seats[4].getPos()
            position = position + Vec3(0, 8, 6.2)
            if base.camera.getH() < 0:
                interval = LerpPosHprInterval(base.camera, 2, position, Vec3(-180, -20, 0), base.camera.getPos(), base.camera.getHpr())
            else:
                interval = LerpPosHprInterval(base.camera, 2, position, Vec3(180, -20, 0), base.camera.getPos(), base.camera.getHpr())
