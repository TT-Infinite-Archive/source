...
class DistributedGameTableAI(DistributedNodeAI):
    ...
    def acceptBoarder(self, avId, seatIndex):
        ...
        if self.game and self.hasPicked == True:
    ...
    def requestPickedGame(self, gameNum):
        avId = self.air.getAvatarIdFromSender()
        if self.hasPicked is False and avId in self.allowPickers:
    ...
    def pickGame(self, gameNum):
        if self.game:
            self.notify.warning('Tried to pick a game when one was already in progress!')
            return
    ...
    def enableTable(self, task):
        self.isAccepting = True
        return task.done

    def handleGameOver(self):
        self.isAccepting = False
        taskMgr.doMethodLater(7, self.enableTable, 'enable-task')
