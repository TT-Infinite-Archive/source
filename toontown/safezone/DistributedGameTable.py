...
class DistributedGameTable(DistributedNode.DistributedNode):
    ...
    def delete(self):
        self.__disableCollisions()
        self.ignore('stoppedAsleep')
        taskMgr.remove(base.localAvatar.uniqueName('sleepwatch'))
    ...
    def emptySlot(self, avId, index, timestamp):
        ...
        if avId in self.cr.doId2do:
            ...
            if base.localAvatar.getDoId() == avId:
                ...
                self.fsm.request('off')
            self.outTrack.append(Func(toon.restoreGoofyEffect, 0.5))
