...
class AvatarChoice(DirectButton):
    ...
    def __handleDelete(self):
        cleanupDialog('globalDialog')
        if self.guildId != 0:
            self.reject = TTDialog.TTGlobalDialog(doneEvent='rejectDone', message=TTLocalizer.AvatarChoiceDeleteRejectGuild % self.name, style=TTDialog.Acknowledge)
            self.reject.show()
            self.accept('rejectDone', self.__handleRejectDone)
        else:
