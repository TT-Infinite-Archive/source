...
def doAnnoyed(toon, volume = 1):
    ...
    if toon.style.getAnimal() == 'bear':
        sfx = loader.loadSfx('phase_3.5/audio/dial/AV_bear_exclaim.ogg')
    else:
        sfx = loader.loadSfx('phase_3.5/audio/sfx/avatar_emotion_angry.ogg')
...
def doUpset(toon, volume = 1):
    ...
    sfx = loader.loadSfx(random.choice(sfxList))
...
def getSingingNote(toon, note, volume = 1):
    ...
    sfx = loader.loadSfx(filePath + filePrefix + speciesName + '_' + note + fileSuffix)
