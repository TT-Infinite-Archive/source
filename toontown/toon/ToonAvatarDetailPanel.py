    ...
    for track in range(0, len(Tracks)):
        ...
        for item in range(0, len(Levels[track])):
            ...
            gagLabel = DirectLabel(image=buttonModel, image_scale=(0.92, 1, 1),
                                ...
                                geom=invModels[track][item], geom_color=geom_color, geom_scale=0.6,
                                relief=None)
            gagLabel.reparentTo(hidden)
            gagButtons[track].append(gagLabel)
    gagGuiLoaded = True


def colorGagButton(panel, track, level, numItems):
    if numItems == 0:
    ...
    yOffset = 0.1
    ySpacing = -0.115
    button = gagButtons[track][level]
    button.reparentTo(panel)
    button.setPos(xOffset + level * xSpacing, 0, yOffset + track * ySpacing)
    button.setProp('image_color', image_color)
    if geom_color:
        button.setProp('geom_color', geom_color)
    else:
        button.setProp('geom_color', Vec4(1, 1, 1, 1))
        button['geom'].clearColor()

def prepareTrackName(panel, i):
    ySpacing = -0.115
    trackLabel = trackLabels[i]
    trackLabel.reparentTo(panel)
    trackLabel.setPos(-0.9, 0, TTLocalizer.TADtrackLabelPosZ + i * ySpacing)
...
class ToonAvatarDetailPanel(DirectFrame):
    ...
    def __init__(self, avId, avName, playerId = None, parent = base.a2dTopRight, **kw):
        ...
        self.bCancel = DirectButton(
            self, image=(buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'),
                         buttons.find('**/CloseBtn_Rllvr')), image_scale=1.1, relief=None,
            text=TTLocalizer.AvatarDetailPanelCancel, text_scale=TTLocalizer.TADPbCancel, text_pos=(0.12, -0.01),
    ...
    def cleanup(self):
        ...
        for track in trackLabels:
            track.reparentTo(hidden)
    ...
    def __showData(self):
        ...
        if online:
            ...
            if self.playerInfo:
                ...
                text = TTLocalizer.AvatarDetailPanelOnlinePlayer % {'location': hoodName,
                                                                    'player': self.playerInfo.playerName} #'district': shardName,
            else:
                text = TTLocalizer.AvatarDetailPanelOnline % {'location': hoodName} #'district': shardName,
        else:
            ...
            if self.avatar.hasTrackAccess(track):
                ...
                for item in range(0, len(Levels[track])):
                    ...
                    if curExp >= level:
                        ...
                        colorGagButton(self, track, item, numItems)
                    else:
                        break
        return
