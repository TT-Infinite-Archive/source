...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def handleLogicalZoneChange(self, newZoneId, oldZoneId):
        ...
        if self.getAdminAccess() < 600 and self.cogIndex != -1:
    ...
    def patch_1(self):
        """
        Patch used to correct the Toon's quests
        """

        # Get the current quests that the toon has:
        quests = self.getQuests()

        # Get the current reward ids for the tier that the toon is in:
        rewardIds = self.getRewardHistory()[1][:]

        # Remove the rewardIds from the list that the toon is currently working on:
        for i in xrange(0, len(quests), 5):
            questDesc = quests[i:i+5]
            if questDesc[3] in rewardIds:
                rewardIds.remove(questDesc[3])

        # Get the current tier of the toon:
        tier = self.getRewardTier()

        gagCarryRewards = []
        questCarryRewards = []

        # Filter out any just-for-fun, gag carry, and quest carry rewards:
        rewards = []
        for rewardId in rewardIds:
            if rewardId in Quests.OptionalRewardTrackDict[tier]:
                continue
            rewardClass = Quests.getReward(rewardId)

            if isinstance(rewardClass, Quests.MaxGagCarryReward):
                gagCarryRewards.append(rewardId)
            elif isinstance(rewardClass, Quests.MaxQuestCarryReward):
                questCarryRewards.append(rewardId)
            else:
                rewards.append(rewardId)

        # Gather all of the required rewards for the previous tiers:
        for t in xrange(tier):
            tierRewards = list(Quests.RequiredRewardTrackDict[t][:])

            finalRewards = []
            for rewardId in tierRewards:
                rewardClass = Quests.getReward(rewardId)

                # Filter out TrackTrainingReward, TrackProgressReward, and TrackCompleteReward
                if isinstance(rewardClass, (Quests.TrackTrainingReward, Quests.TrackProgressReward,
                                            Quests.TrackCompleteReward)):
                    continue

                # We will calculate the biggest max gag carry reward later:
                elif isinstance(rewardClass, Quests.MaxGagCarryReward):
                    gagCarryRewards.append(rewardId)

                # We will calculate the biggest max quest carry reward later:
                elif isinstance(rewardClass, Quests.MaxQuestCarryReward):
                    questCarryRewards.append(rewardId)
                else:
                    finalRewards.append(rewardId)

            # Add the rewards on to the beginning of the rewards list
            rewards = finalRewards + rewards

        # Calculate the max gag carry reward:
        gagCarryReward = None
        for rewardId in gagCarryRewards:
            if gagCarryReward is None:
                gagCarryReward = (rewardId, Quests.RewardDict[rewardId][1])
                continue
            rewardInfo = Quests.RewardDict[rewardId]
            if rewardInfo[1] > gagCarryReward[1]:
                gagCarryReward = (rewardId, Quests.RewardDict[rewardId][1])
        if gagCarryReward is not None:
            rewards.append(gagCarryReward[0])

        # Calculate the max quest carry reward:
        questCarryReward = None
        for rewardId in questCarryRewards:
            if questCarryReward is None:
                questCarryReward = (rewardId, Quests.RewardDict[rewardId][1])
                continue
            rewardInfo = Quests.RewardDict[rewardId]
            if rewardInfo[1] > questCarryReward[1]:
                questCarryReward = (rewardId, Quests.RewardDict[rewardId][1])
        if questCarryReward is not None:
            rewards.append(questCarryReward[0])

        # Gather all of the hp gained in racing, fishing, golfing, and suits.
        extraHp = 0
        ...
        for value in self.getKartingTrophies():
            if value:
                trophies += 1
        extraHp += int(trophies/10)

        extraHp += len(self.getFishingTrophies())

        trophies = 0
        for value in GolfGlobals.calcTrophyListFromHistory(self.golfHistory):
            if value:
                trophies += 1
        extraHp += int(trophies/10)

        for x in xrange(4):
            extraHp += self.getCogTypes()[x]

        # Get the hp of the current toon:
        hp = self.getHp()

        # Reset the toon to 15 laff + the extraHp:
        self.b_setMaxHp(15+extraHp)

        # Apply all of the rewards on the toon:
        for rewardId in rewards:
            rewardClass = Quests.getReward(rewardId)
            rewardClass.sendRewardAI(self)

        # Set the toon to the hp it had before we applied the rewards:
        self.b_setHp(min(hp, self.getMaxHp()))
