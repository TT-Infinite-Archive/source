...
class DistributedGolfCourseAI(DistributedObjectAI.DistributedObjectAI, FSM):
    ...
    def enterWaitReward(self):
        ...
        if self.air.wantToonStats:
            self.air.statManager.handleGolfCompleted(self.avIdList, self.getCourseId())

        if self.air.wantGuilds and self.air.wantGuildQuests:
            self.air.guildManager.handleGolfCompleted(self.avIdList, self.getCourseId())
    ...
    def calcRankings(self):
        ...
        if GolfGlobals.TIME_TIE_BREAKER:
            # Score compare with time
            totalScores.sort(key=lambda x: (x[1], x[2]))
        else:
            ...
            if av:
                ...
                for index in range(len(oldTrophies)):
                    if not oldTrophies[index] and endingTrophies[index]:
                        ...
                        if self.air.wantToonStats:
                            self.air.statManager.handleGolfTrophyAcquired(avId)
