...
class QuestMap(DirectFrame):
    ...
    def __init__(self, av, **kw):
        ...
        self.container.setColorScale(1.0, 1.0, 1.0, 0.75)
    ...
    def putBuildingMarker(self, pos, mapIndex=None, isSuitBlock=False):
        ...
        marker['image'] = iconNP
        if isSuitBlock:
            marker['image_color'] = (0.45, 0.45, 0.45, 1)
        else:
    ...
    def putCogMarker(self, blockNumber, track, pos):
        self.notify.debug('putCogMarker(%s, %s, %s)' % (blockNumber, track, pos))
    ...
    def updateQuestInfo(self):
        ...
        for marker in self.cogMarkers:
            marker.destroy()

        self.buildingMarkers = []
        self.cogMarkers = []

        # Check if we have a loader; if we don't, we cant continue:
        if not hasattr(base.cr.playGame.hood, 'loader'):
            # Lets try updating our info at a later date:
            taskMgr.doMethodLater(0.2, self.updateQuestInfo, 'update-quest-info', extraArgs=[])
            return
    ...
    def acceptOnscreenHooks(self):
        taskMgr.remove('quest-map-toggle-onscreen-hooks')
        taskMgr.doMethodLater(1, self.toggleOnscreenHooks, 'quest-map-toggle-onscreen-hooks')
