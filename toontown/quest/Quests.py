...
def seedRandomGen(npcId, avId, tier, rewardHistory):
    QuestRandGen.seed(npcId * 100 + avId + tier + len(rewardHistory) + time.time() // 3600)
...
def npcMatches(toNpcId, npc):
    return toNpcId == npc.getNpcId() or toNpcId == Any\
           or toNpcId == ToonHQ and npc.getHq() or toNpcId == ToonTailor and npc.getTailor()
...
class NewbieQuest:
    ...
    def getNumNewbies(self, avId, avList):
        ...
        number = 0
        for avatarId in avList:
            avatar = None

            if process == 'client':
                avatar = base.cr.doId2do.get(avatarId)
            else:
...
class CogQuest(LocationBasedQuest):
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        return (questCogType == Any or questCogType == cogDict['type']) and \
               (avId in cogDict['activeToons']) and self.isLocationMatch(zoneId)
...
class CogTrackLevelQuest(CogQuest):
    ...
    def __init__(self, id, quest):
        CogQuest.__init__(self, id, quest)
        if self.__class__ == CogTrackLevelQuest:
            self.checkNumCogs(self.quest[1])
            self.checkCogTrack(self.quest[2])
    ...
    def getProgressString(self, avatar, questDesc):
        if self.getCompletionStatus(avatar, questDesc) == COMPLETE:
            ...
            return TTLocalizer.QuestsCogTrackLevelQuestProgress % {'progress': questDesc[4],
             'numCogs': self.getNumCogs()}
    ...
    def getObjectiveStrings(self):
        ...
        if numCogs == 1:
            ...
            text = TTLocalizer.QuestsCogTrackLevelDefeatDesc % {'numCogs': numCogs,
             'level': level,
             'trackName': self.trackNamesP[track]}
        return (text,)

    def getString(self):
        return TTLocalizer.QuestsCogTrackLevelQuestDefeat % self.getObjectiveStrings()[0]
    ...
    def getSCStrings(self, toNpcId, progress):
        ...
        if numCogs == 1:
            cogText = self.trackNamesS[track]
            text = TTLocalizer.QuestsCogTrackLevelQuestSCStringS
        else:
            cogText = self.trackNamesP[track]
            text = TTLocalizer.QuestsCogTrackLevelQuestSCStringP
        cogLocName = self.getLocationName()
        return text % {'cogText': cogText,
         'level': level,
         'cogLoc': cogLocName}
    ...
    def getHeadlineString(self):
        return TTLocalizer.QuestsCogTrackLevelQuestHeadline

    def doesCogCount(self, avId, cogDict, zoneId, avList):
        questCogTrack = self.getCogTrack()
        questCogLevel = self.getCogLevel()
        return (questCogTrack == cogDict['track']) and (questCogLevel <= cogDict['level']) and\
               (avId in cogDict['activeToons']) and self.isLocationMatch(zoneId)
...
class BuildingNewbieQuest(BuildingQuest, NewbieQuest):
    ...
    def doesBuildingCount(self, avId, avList):
        ...
        for toon in avList:
            avIds.append(toon.doId)

        return self.getNumNewbies(avId, avIds)
...
class RecoverItemQuest(LocationBasedQuest):
    def __init__(self, id, quest):
        ...
        if len(self.quest) > 6:
            self.checkCogLevel(self.quest[6])
        if len(self.quest) > 5:
    ...
    def doesCogCount(self, avId, cogDict, zoneId, avList):
        ...
        if len(self.quest) > 6:
            questCogLevel = self.getCogLevel()
            return (questCogType == Any or questCogType == cogDict[self.getHolderType()]) and avId in cogDict['activeToons'] and self.isLocationMatch(zoneId) and questCogLevel <= cogDict['level']
        return (questCogType == Any or questCogType == cogDict[self.getHolderType()]) and avId in cogDict['activeToons'] and self.isLocationMatch(zoneId)
...
QuestDict = {
    ...
    110: (TT_TIER, Cont, (TrolleyQuest,), Any, ToonHQ, NA, (130, 140), DefaultDialog),
    ...
    130: (TT_TIER, Cont, (DeliverItemQuest, 6), ToonHQ, 2003, NA, 131, DefaultDialog),
    131: (TT_TIER, Cont, (RecoverItemQuest, ToontownGlobals.ToontownCentral, 1, 3, VeryEasy, Any, 'type'), 2003, 2003, NA, 175, DefaultDialog),
    140: (TT_TIER, Cont, (DeliverItemQuest, 4), ToonHQ, 2005, NA, 141, DefaultDialog),
    141: (TT_TIER, Cont, (RecoverItemQuest, ToontownGlobals.ToontownCentral, 1, 1, VeryEasy, Any, 'type'), 2005, 2005, NA, 175, DefaultDialog),
    ...
    160: (TT_TIER, OBSOLETE, (CogTrackQuest, ToontownGlobals.ToontownCentral, 3, 'c'), Same, ToonHQ, NA, 175, DialogDict[160]),
    161: (TT_TIER, OBSOLETE, (CogTrackQuest, ToontownGlobals.ToontownCentral, 3, 'l'), Same, ToonHQ, NA, 175, DialogDict[161]),
    162: (TT_TIER, OBSOLETE, (CogTrackQuest, ToontownGlobals.ToontownCentral, 3, 's'), Same, ToonHQ, NA, 175, DialogDict[162]),
    163: (TT_TIER, OBSOLETE, (CogTrackQuest, ToontownGlobals.ToontownCentral, 3, 'm'), Same, ToonHQ, NA, 175, DialogDict[163]),
    175: (TT_TIER, Cont, (PhoneQuest,), Same, ToonHQ, 100, NA, DialogDict[175]),
    164: (TT_TIER + 1, Start, (VisitQuest,), Any, 2001, NA, 165, DialogDict[164]),
    165: (TT_TIER + 1, Start, (CogQuest, Anywhere, 4, Any), 2001, Same, NA, (166, 167, 168, 169), DialogDict[165]),
    166: (TT_TIER + 1, Cont, (CogTrackQuest, Anywhere, 4, 'c'), Same, Same, NA, 170, DialogDict[166]),
    167: (TT_TIER + 1, Cont, (CogTrackQuest, Anywhere, 4, 'l'), Same, Same, NA, 170, DialogDict[167]),
    168: (TT_TIER + 1, Cont, (CogTrackQuest, Anywhere, 4, 's'), Same, Same, NA, 170, DialogDict[168]),
    169: (TT_TIER + 1, Cont, (CogTrackQuest, Anywhere, 4, 'm'), Same, Same, NA, 170, DialogDict[169]),
    170: (TT_TIER + 1, Cont, (VisitQuest,), Same, 2005, NA, 400, DialogDict[170]),
    400: (TT_TIER + 1, Cont, (TrackChoiceQuest, ToontownBattleGlobals.SOUND_TRACK, ToontownBattleGlobals.HEAL_TRACK), Same, Same, 400, NA, DialogDict[400]),
    1001: (TT_TIER + 2, Start, (CogQuest, ToontownGlobals.ToontownCentral, 4, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    1002: (TT_TIER + 2, Start, (CogQuest, ToontownGlobals.ToontownCentral, 7, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    1003: (TT_TIER + 2, Start, (CogQuest, ToontownGlobals.ToontownCentral, 10, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    1004: (TT_TIER + 2, Start, (CogQuest, ToontownGlobals.ToontownCentral, 14, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    1005: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'f'), Any, ToonHQ, Any, NA, DefaultDialog),
    1006: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'p'), Any, ToonHQ, Any, NA, DefaultDialog),
    1007: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'bf'), Any, ToonHQ, Any, NA, DefaultDialog),
    1008: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'b'), Any, ToonHQ, Any, NA, DefaultDialog),
    1009: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'sc'), Any, ToonHQ, Any, NA, DefaultDialog),
    1010: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'pp'), Any, ToonHQ, Any, NA, DefaultDialog),
    1011: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'cc'), Any, ToonHQ, Any, NA, DefaultDialog),
    1012: (TT_TIER + 2, Start, (CogQuest, Anywhere, 3, 'tm'), Any, ToonHQ, Any, NA, DefaultDialog),
    1013: (TT_TIER + 2, Start, (CogQuest, Anywhere, 5, 'f'), Any, ToonHQ, Any, NA, DefaultDialog),
    1014: (TT_TIER + 2, Start, (CogQuest, Anywhere, 5, 'p'), Any, ToonHQ, Any, NA, DefaultDialog),
    1015: (TT_TIER + 2, Start, (CogQuest, Anywhere, 5, 'bf'), Any, ToonHQ, Any, NA, DefaultDialog),
    1016: (TT_TIER + 2, Start, (CogQuest, Anywhere, 5, 'b'), Any, ToonHQ, Any, NA, DefaultDialog),
    ...
    1021: (TT_TIER + 2, Start, (CogLevelQuest, ToontownGlobals.ToontownCentral, 2, 3), Any, ToonHQ, Any, NA, DefaultDialog),
    1022: (TT_TIER + 2, Start, (CogLevelQuest, ToontownGlobals.ToontownCentral, 6, 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1023: (TT_TIER + 2, Start, (CogLevelQuest, ToontownGlobals.ToontownCentral, 10, 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1024: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.SillyStreet, 4, 'c'), Any, ToonHQ, Any, NA, DefaultDialog),
    1025: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.LoopyLane, 4, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    1026: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.PunchlinePlace, 4, 'm'), Any, ToonHQ, Any, NA, DefaultDialog),
    1027: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 5, 'm'), Any, ToonHQ, Any, NA, DefaultDialog),
    1028: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 5, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    1029: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 5, 'c'), Any, ToonHQ, Any, NA, DefaultDialog),
    1030: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 5, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    1031: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 7, 'm'), Any, ToonHQ, Any, NA, DefaultDialog),
    1032: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 7, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    1033: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 7, 'c'), Any, ToonHQ, Any, NA, DefaultDialog),
    1034: (TT_TIER + 2, Start, (CogTrackQuest, ToontownGlobals.ToontownCentral, 7, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    1035: (TT_TIER + 2, Start, (CogTrackLevelQuest, ToontownGlobals.ToontownCentral, 5, 'm', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1036: (TT_TIER + 2, Start, (CogTrackLevelQuest, ToontownGlobals.ToontownCentral, 5, 's', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1037: (TT_TIER + 2, Start, (CogTrackLevelQuest, ToontownGlobals.ToontownCentral, 5, 'c', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1038: (TT_TIER + 2, Start, (CogTrackLevelQuest, ToontownGlobals.ToontownCentral, 5, 'l', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1039: (TT_TIER + 2, Start, (VisitQuest,), Any, 2135, NA, (1041, 1042, 1043), DialogDict[1039]),
    1040: (TT_TIER + 2, Start, (VisitQuest,), Any, 2207, NA, (1041, 1042, 1043), DialogDict[1040]),
    1041: (TT_TIER + 2, Cont, (VisitQuest,), Same, 2211, NA, 1044, DialogDict[1041]),
    1042: (TT_TIER + 2, Cont, (VisitQuest,), Same, 2209, NA, 1044, DialogDict[1042]),
    1043: (TT_TIER + 2, Cont, (VisitQuest,), Same, 2210, NA, 1044, DialogDict[1043]),
    1044: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 7, 7, Easy, 2, 'level'), Same, Same, NA, 1045, DialogDict[1044]),
    1045: (TT_TIER + 2, Cont, (DeliverItemQuest, 8), Same, ToonHQ, 300, NA, DialogDict[1045]),
    1046: (TT_TIER + 2, Start, (VisitQuest,), Any, 2127, NA, 1047, DialogDict[1046]),
    1047: (TT_TIER + 2, Start, (RecoverItemQuest, Anywhere, 5, 9, VeryEasy, 'm', 'track'), 2127, Same, NA, 1048, DialogDict[1047]),
    1048: (TT_TIER + 2, Cont, (DeliverItemQuest, 9), Same, 2131, NA, 1049, DialogDict[1048]),
    1049: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 10, 2007, VeryEasy, 3, 'level'), Same, Same, NA, 1053, DialogDict[1049]),
    1053: (TT_TIER + 2, Cont, (DeliverItemQuest, 9), Same, 2127, 700, NA, DialogDict[1053]),
    1054: (TT_TIER + 2, Start, (VisitQuest,), Any, 2128, NA, 1055, DialogDict[1054]),
    1055: (TT_TIER + 2, Start, (RecoverItemQuest, ToontownGlobals.SillyStreet, 4, 10, Easy, AnyFish), 2128, Same, NA, 1056, DialogDict[1055]),
    1056: (TT_TIER + 2, Cont, (VisitQuest,), Same, 2213, NA, 1057, DialogDict[1056]),
    1057: (TT_TIER + 2, Cont, (CogLevelQuest, ToontownGlobals.LoopyLane, 10, 2), Same, Same, NA, 1058, DialogDict[1057]),
    1058: (TT_TIER + 2, Cont, (DeliverItemQuest, 11), Same, 2128, 200, NA, DialogDict[1058]),
    1059: (TT_TIER + 2, Start, (VisitQuest,), Any, 2302, NA, 1060, DialogDict[1059]),
    1060: (TT_TIER + 2, Start, (RecoverItemQuest, Anywhere, 3, 12, Medium, AnyFish), 2302, Same, NA, 1062, DialogDict[1060]),
    1061: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.ToontownCentral, 6, 'p'), Same, Same, 101, NA, DialogDict[1061]),
    1062: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.ToontownCentral, 6, 'b'), Same, Same, 101, NA, DialogDict[1062]),
    900: (TT_TIER + 3, Start, (VisitQuest,), Any, 2201, NA, 1063, DialogDict[900]),
    1063: (TT_TIER + 3, Start, (RecoverItemQuest, Anywhere, 1, 13, Medium, 3, 'level'), 2201, Same, NA, 1067, DialogDict[1063]),
    1067: (TT_TIER + 3, Cont, (DeliverItemQuest, 13), Same, 2112, NA, 1068, DialogDict[1067]),
    1068: (TT_TIER + 3, Cont, (CogQuest, ToontownGlobals.ToontownCentral, 10, Any), Same, Same, NA, (1069, 1070, 1071), DialogDict[1068]),
    1069: (TT_TIER + 3, Cont, (RecoverItemQuest, Anywhere, 1, 13, Medium, 'm', 'track', 3), Same, Same, NA, 1072, DialogDict[1069]),
    1070: (TT_TIER + 3, Cont, (RecoverItemQuest, Anywhere, 1, 13, Medium, 's', 'track', 3), Same, Same, NA, 1072, DialogDict[1070]),
    1071: (TT_TIER + 3, Cont, (RecoverItemQuest, Anywhere, 1, 13, Medium, 'c', 'track', 3), Same, Same, NA, 1072, DialogDict[1071]),
    1072: (TT_TIER + 3, Cont, (DeliverItemQuest, 13), Same, 2301, NA, 1073, DialogDict[1072]),
    1073: (TT_TIER + 3, Cont, (VisitQuest,), Any, 2201, NA, 1074, DialogDict[1073]),
    1074: (TT_TIER + 3, Cont, (RecoverItemQuest, Anywhere, 1, 13, Hard, Any), Same, Same, NA, 1075, DialogDict[1074]),
    1075: (TT_TIER + 3, Cont, (DeliverItemQuest, 13), Same, 2301, 900, NA, DialogDict[1075]),
    1076: (TT_TIER + 2, Start, (VisitQuest,), Any, 2217, NA, 1077, DialogDict[1076]),
    1077: (TT_TIER + 2, Start, (RecoverItemQuest, Anywhere, 1, 14, Medium, Any), 2217, Same, NA, 1078, DialogDict[1077]),
    1078: (TT_TIER + 2, Cont, (DeliverItemQuest, 14), Same, 2302, NA, (1079, 1092), DialogDict[1078]),
    1079: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 15, Easy, 'f'), Same, 2217, NA, 1080, DialogDict[1079]),
    1092: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 15, Easy, 'sc'), Same, 2217, NA, 1080, DialogDict[1092]),
    1080: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.ToontownCentral, 4, 15, Easy, AnyFish), Same, Same, 500, NA, DialogDict[1080]),
    1081: (TT_TIER + 2, Start, (VisitQuest,), Any, 2208, NA, 1082, DialogDict[1081]),
    1082: (TT_TIER + 2, Start, (RecoverItemQuest, ToontownGlobals.SillyStreet, 1, 16, Easy, 's', 'track', 3), 2208, Same, NA, 1083, DialogDict[1082]),
    1083: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.LoopyLane, 1, 17, Medium, 'l', 'track', 3), Same, Same, NA, 1084, DialogDict[1083]),
    1084: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.PunchlinePlace, 1, 18, VeryEasy, 'm', 'track', 3), Same, Same, 102, NA, DialogDict[1084]),
    1085: (TT_TIER + 2, Start, (VisitQuest,), Any, 2003, NA, 1086, DialogDict[1085]),
    1086: (TT_TIER + 2, Start, (RecoverItemQuest, Anywhere, 12, 2007, Easy, 2, 'level'), 2003, Same, NA, 1089, DialogDict[1086]),
    1089: (TT_TIER + 2, Cont, (DeliverItemQuest, 19), Same, ToonHQ, 100, NA, DialogDict[1089]),
    1090: (TT_TIER + 2, Start, (VisitQuest,), Any, 2119, NA, 1091, DialogDict[1090]),
    1091: (TT_TIER + 2, Start, (CogLevelQuest, ToontownGlobals.ToontownCentral, 8, 2), 2119, ToonHQ, 101, NA, DialogDict[1091]),
    ...
    1105: (TT_TIER + 2, Start, (CogQuest, Anywhere, 1, 'f'), Any, ToonHQ, NA, 1106, DefaultDialog),
    1106: (TT_TIER + 2, Cont, (CogQuest, Anywhere, 2, 'p'), Same, Same, Any, NA, DefaultDialog),
    1107: (TT_TIER + 2, Start, (CogQuest, Anywhere, 1, 'bf'), Any, ToonHQ, NA, 1108, DefaultDialog),
    1108: (TT_TIER + 2, Cont, (CogQuest, Anywhere, 2, 'b'), Same, Same, Any, NA, DefaultDialog),
    1109: (TT_TIER + 2, Start, (CogQuest, Anywhere, 1, 'sc'), Any, ToonHQ, NA, 1110, DefaultDialog),
    1110: (TT_TIER + 2, Cont, (CogQuest, Anywhere, 2, 'pp'), Same, Same, Any, NA, DefaultDialog),
    1111: (TT_TIER + 2, Start, (CogQuest, Anywhere, 1, 'cc'), Any, ToonHQ, NA, 1112, DefaultDialog),
    1112: (TT_TIER + 2, Cont, (CogQuest, Anywhere, 2, 'tm'), Same, Same, Any, NA, DefaultDialog),
    ...
    1213: (TT_TIER + 2, Start, (CogTrackLevelQuest, Anywhere, 10, 's', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1214: (TT_TIER + 2, Start, (CogTrackLevelQuest, Anywhere, 10, 'm', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1215: (TT_TIER + 2, Start, (CogTrackLevelQuest, Anywhere, 10, 'l', 2), Any, ToonHQ, Any, NA, DefaultDialog),
    1216: (TT_TIER + 2, Start, (CogTrackLevelQuest, Anywhere, 10, 'c', 2), Any, ToonHQ, Any, NA, DefaultDialog),

    # New TTC Task are here.
    # Tier, Start Flag, Quest Class, FromNPC, ToNPC, Reward, Next ID, Dialog

    1300: (TT_TIER + 2, Start, (VisitQuest,), Any, 2124, NA, 1301, DialogDict[1300]),
    1301: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.SillyStreet, 6, 21, Easy, Any, 'type'), Same, Same, NA, 1302, DialogDict[1301]),
    1302: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.LoopyLane, 3, 22, Medium, Any, 'type'), Same, Same, 200, NA, DialogDict[1302]),

    1303: (TT_TIER + 2, Start, (VisitQuest,), Any, 2121, NA, 1304, DefaultDialog),
    1304: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 5, 23, Medium, AnyFish), Same, Same, NA, 1305, DialogDict[1304]),
    1305: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 5, 24, Easy, 2, 'level'), Same, Same, 200, NA, DialogDict[1305]),

    1306: (TT_TIER + 2, Start, (VisitQuest,), Any, 2132, NA, (1307, 1308), DefaultDialog),
    1307: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 2, 25, Easy, 'cc', 'type'), Same, Same, NA, 1309, DialogDict[1307]),
    1308: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 2, 25, Easy, 'sc', 'type'), Same, Same, NA, 1309, DialogDict[1307]),
    1309: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.SillyStreet, 5, Any), Same, Same, NA, (1310, 1311), DialogDict[1308]),
    1310: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 26, Medium, 's', 'track'), Same, Same, 300, NA, DialogDict[1310]),
    1311: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 26, Medium, 'm', 'track'), Same, Same, 300, NA, DialogDict[1310]),

    1312: (TT_TIER + 2, Start, (VisitQuest,), Any, 2133, NA, (1313, 1314), DefaultDialog),
    1313: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 12, 27, 90, 's', 'track'), Same, Same, NA, 1315, DialogDict[1313]),
    1314: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 12, 27, 90, 'm', 'track'), Same, Same, NA, 1315, DialogDict[1313]),
    1315: (TT_TIER + 2, Cont, (DeliverGagQuest, 24, ToontownBattleGlobals.SQUIRT_TRACK, 1), Same, Same, 100, NA, DialogDict[1315]),

    1316: (TT_TIER + 2, Start, (VisitQuest,), Any, 2101, NA, (1317, 1318), DefaultDialog),
    1317: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 28, Easy, 'pp', 'type'), Same, Same, NA, 1319, DialogDict[1317]),
    1318: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 28, Easy, 'ym', 'type'), Same, Same, NA, 1320, DialogDict[1317]),
    1319: (TT_TIER + 2, Cont, (CogTrackLevelQuest, ToontownGlobals.ToontownCentral, 5, 'm', 2), Same, Same, 100, NA, DialogDict[1319]),
    1320: (TT_TIER + 2, Cont, (CogTrackLevelQuest, ToontownGlobals.ToontownCentral, 2, 'c', 3), Same, Same, 100, NA, DialogDict[1319]),

    1321: (TT_TIER + 2, Start, (VisitQuest,), Any, 2215, NA, 1322, DefaultDialog),
    1322: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.LoopyLane, 5, 29, Easy, 'l', 'track'), Same, Same, NA, 1323, DialogDict[1322]),
    1323: (TT_TIER + 2, Cont, (DeliverGagQuest, 10, ToontownBattleGlobals.SQUIRT_TRACK, 1), Same, Same, 100, NA, DialogDict[1323]),

    1324: (TT_TIER + 2, Start, (VisitQuest,), Any, 2224, NA, 1325, DefaultDialog),
    1325: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.LoopyLane, 1, 30, Easy, 'l', 'track'), Same, Same, NA, 1326, DialogDict[1325]),
    1326: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.SillyStreet, 2, 30, Easy, 'c', 'track'), Same, Same, NA, 1327, DialogDict[1326]),
    1327: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.PunchlinePlace, 3, 30, Easy, 'm', 'track'), Same, Same, 101, NA, DialogDict[1327]),

    1328: (TT_TIER + 2, Start, (VisitQuest,), Any, 2222, NA, 1329, DefaultDialog),
    1329: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.LoopyLane, 3, 30, 85, 'l', 'track', 2), Same, Same, 101, NA, DialogDict[1325]),

    1330: (TT_TIER + 2, Start, (VisitQuest,), Any, 2219, NA, 1331, DefaultDialog),
    1331: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.LoopyLane, 12, Any), Same, Same, NA, 1332, DialogDict[1331]),
    1332: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.LoopyLane, 3, 31, 85, 'bf', 'type'), Same, Same, 101, NA, DialogDict[1332]),

    1333: (TT_TIER + 2, Start, (VisitQuest,), Any, 2303, NA, 1334, DefaultDialog),
    1334: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 32, 85, 'b', 'type'), Same, Same, NA, (1335, 1336), DialogDict[1334]),
    1335: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 33, 85, 'tm', 'type'), Same, Same, NA, (1337, 1338), DefaultDialog),
    1336: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 33, 85, 'p', 'type'), Same, Same, NA, (1337, 1338), DefaultDialog),
    1337: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 34, 85, 'dt', 'type'), Same, Same, 102, NA, DefaultDialog),
    1338: (TT_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 34, 85, 'ym', 'type'), Same, Same, 102, NA, DefaultDialog),

    1339: (TT_TIER + 2, Start, (VisitQuest,), Any, 2314, NA, 1340, DefaultDialog),
    1340: (TT_TIER + 2, Cont, (DeliverGagQuest, 1, ToontownBattleGlobals.THROW_TRACK, 0), Same, Same, NA, 1341, DialogDict[1340]),
    1341: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.PunchlinePlace, 3, Any), Same, Same, NA, 1342, DialogDict[1341]),
    1342: (TT_TIER + 2, Cont, (DeliverGagQuest, 5, ToontownBattleGlobals.THROW_TRACK, 1), Same, Same, NA, 1343, DialogDict[1342]),
    1343: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.LoopyLane, 6, Any), Same, Same, NA, 1344, DialogDict[1343]),
    1344: (TT_TIER + 2, Cont, (DeliverGagQuest, 10, ToontownBattleGlobals.THROW_TRACK, 2), Same, Same, NA, 1345, DialogDict[1344]),
    1345: (TT_TIER + 2, Cont, (CogQuest, ToontownGlobals.SillyStreet, 9, Any), Same, Same, 102, NA, DialogDict[1345]),

    1346: (TT_TIER + 2, Start, (VisitQuest,), Any, 2315, NA, 1347, DefaultDialog),
    1347: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.PunchlinePlace, 4, 35, 95, 's', 'track', 2), Same, Same, 100, NA, DialogDict[1347]),

    1348: (TT_TIER + 2, Start, (VisitQuest,), Any, 2316, NA, 1349, DefaultDialog),
    1349: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.PunchlinePlace, 1, 36, 70, 3, 'level'), Same, Same, NA, 1350, DialogDict[1349]),
    1350: (TT_TIER + 2, Cont, (VisitQuest,), 2316, 2122, NA, 1351, DialogDict[1350]),
    1351: (TT_TIER + 2, Cont, (RecoverItemQuest, ToontownGlobals.SillyStreet, 5, 37, 70, 2, 'level'), Same, Same, NA, 1352, DialogDict[1351]),
    1352: (TT_TIER + 2, Cont, (DeliverItemQuest, 37), 2122, 2316, NA, 1353, DialogDict[1352]),
    1353: (TT_TIER + 2, Cont, (CogTrackLevelQuest, Anywhere, 5, 'm', 2), Same, Same, 102, NA, DialogDict[1353]),

    401: (DD_TIER, Start, (TrackChoiceQuest, ToontownBattleGlobals.DROP_TRACK, ToontownBattleGlobals.LURE_TRACK), Any, ToonHQ, 400, NA, DialogDict[401]),
    2001: (DD_TIER, Start, (CogQuest, ToontownGlobals.LighthouseLane, 5, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2002: (DD_TIER, Start, (CogQuest, ToontownGlobals.SeaweedStreet, 8, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2003: (DD_TIER, Start, (CogQuest, Anywhere, 10, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2004: (DD_TIER, Start, (CogQuest, Anywhere, 12, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2005: (DD_TIER, Start, (CogQuest, Anywhere, 14, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2006: (DD_TIER, Start, (CogQuest, Anywhere, 16, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2007: (DD_TIER, Start, (CogQuest, Anywhere, 18, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2008: (DD_TIER, Start, (CogQuest, Anywhere, 20, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2009: (DD_TIER, Start, (CogQuest, ToontownGlobals.BarnacleBoulevard, 12, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2010: (DD_TIER, Start, (CogLevelQuest, Anywhere, 2, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    2011: (DD_TIER, Start, (CogLevelQuest, Anywhere, 3, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    2012: (DD_TIER, Start, (CogLevelQuest, Anywhere, 4, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    2013: (DD_TIER, Start, (CogLevelQuest, Anywhere, 7, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    2014: (DD_TIER, Start, (CogLevelQuest, Anywhere, 10, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    2015: (DD_TIER, Start, (CogLevelQuest, Anywhere, 5, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    2816: (DD_TIER, Start, (CogLevelQuest, Anywhere, 7, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    ...
    2819: (DD_TIER, Start, (CogLevelQuest, Anywhere, 8, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    2020: (DD_TIER, Start, (CogQuest, Anywhere, 5, Any), Any, ToonHQ, NA, 2021, DefaultDialog),
    ...
    2102: (DD_TIER + 1, Start, (CogQuest, ToontownGlobals.DonaldsDock, 10, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    2103: (DD_TIER + 1, Start, (CogQuest, ToontownGlobals.DonaldsDock, 15, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    ...
    2131: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 4, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    2132: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 6, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    2133: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 5, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    2134: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 7, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    2135: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 4, 6), Any, ToonHQ, Any, NA, DefaultDialog),
    2136: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 4, 6), Any, ToonHQ, Any, NA, DefaultDialog),
    2137: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 1, 7), Any, ToonHQ, Any, NA, DefaultDialog),
    2138: (DD_TIER + 1, Start, (CogLevelQuest, ToontownGlobals.DonaldsDock, 2, 7), Any, ToonHQ, Any, NA, DefaultDialog),
    2139: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 12, 'm'), Any, ToonHQ, Any, NA, DefaultDialog),
    2140: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 5, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    2141: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 12, 'c'), Any, ToonHQ, Any, NA, DefaultDialog),
    2142: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 10, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    2143: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 16, 'm'), Any, ToonHQ, Any, NA, DefaultDialog),
    2144: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 10, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    2145: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 16, 'c'), Any, ToonHQ, Any, NA, DefaultDialog),
    2146: (DD_TIER + 1, Start, (CogTrackQuest, ToontownGlobals.DonaldsDock, 16, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    2147: (DD_TIER + 1, Start, (CogTrackQuest, Anywhere, 20, 'm'), Any, ToonHQ, Any, NA, DefaultDialog),
    2148: (DD_TIER + 1, Start, (CogTrackQuest, Anywhere, 20, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    2149: (DD_TIER + 1, Start, (CogTrackQuest, Anywhere, 20, 'c'), Any, ToonHQ, Any, NA, DefaultDialog),
    2150: (DD_TIER + 1, Start, (CogTrackQuest, Anywhere, 20, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    ...
    2155: (DD_TIER + 1, Start, (BuildingQuest, Anywhere, 2, 'm', 1), Any, ToonHQ, Any, NA, DefaultDialog),
    2156: (DD_TIER + 1, Start, (BuildingQuest, Anywhere, 2, 's', 1), Any, ToonHQ, Any, NA, DefaultDialog),
    2157: (DD_TIER + 1, Start, (BuildingQuest, Anywhere, 2, 'c', 1), Any, ToonHQ, Any, NA, DefaultDialog),
    2158: (DD_TIER + 1, Start, (BuildingQuest, Anywhere, 2, 'l', 1), Any, ToonHQ, Any, NA, DefaultDialog),
    2159: (DD_TIER + 1, Start, (DeliverGagQuest, 20, ToontownBattleGlobals.THROW_TRACK, 2), Any, Any, NA, 2160, DefaultDialog),
    2160: (DD_TIER + 1, Cont, (DeliverGagQuest, 20, ToontownBattleGlobals.SQUIRT_TRACK, 2), Same, Same, Any, NA, DefaultDialog),
    2161: (DD_TIER + 1, Start, (DeliverGagQuest, 1, ToontownBattleGlobals.SQUIRT_TRACK, 3), Any, Any, NA, 2162, DefaultDialog),
    2162: (DD_TIER + 1, Cont, (DeliverGagQuest, 2, ToontownBattleGlobals.THROW_TRACK, 3), Same, Same, Any, NA, DefaultDialog),
    2201: (DD_TIER + 1, Start, (VisitQuest,), Any, 1101, NA, 2202, DialogDict[2201]),
    2202: (DD_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 2001, Medium, 'pp'), 1101, Same, 101, NA, DialogDict[2202]),
    2203: (DD_TIER + 1, Start, (VisitQuest,), Any, 1102, NA, 2204, DialogDict[2203]),
    2204: (DD_TIER + 1, Start, (DeliverItemQuest, 2002), 1102, 1104, NA, 2205, DialogDict[2204]),
    2205: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 4, 2003, Medium, 'f'), Same, Same, NA, 2206, DialogDict[2205]),
    2206: (DD_TIER + 1, Cont, (DeliverItemQuest, 2004), Same, 1102, 201, NA, DialogDict[2206]),
    2207: (DD_TIER + 1, Start, (VisitQuest,), Any, 1201, NA, 2208, DialogDict[2207]),
    2208: (DD_TIER + 1, Start, (RecoverItemQuest, Anywhere, 3, 2005, Medium, 'bs'), 1201, Same, 701, NA, DialogDict[2208]),
    2209: (DD_TIER + 1, Start, (VisitQuest,), Any, 1302, NA, 2210, DialogDict[2209]),
    2210: (DD_TIER + 1, Start, (VisitQuest,), 1302, 1301, NA, 2211, DialogDict[2210]),
    2211: (DD_TIER + 1, Cont, (CogQuest, ToontownGlobals.DonaldsDock, 5, 'mm'), Same, Same, NA, 2212, DialogDict[2211]),
    2212: (DD_TIER + 1, Cont, (DeliverItemQuest, 2006), Same, 1302, NA, 2213, DialogDict[2212]),
    2213: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1202, NA, 2214, DialogDict[2213]),
    2214: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.DonaldsDock, 7, 2007, 40, Any), Same, Same, NA, 2215, DialogDict[2214]),
    2215: (DD_TIER + 1, Cont, (DeliverItemQuest, 2008), Same, 1302, 301, NA, DialogDict[2215]),
    ...
    901: (DD_TIER + 2, Start, (VisitQuest,), Any, 1203, NA, 2902, DialogDict[901]),
    2902: (DD_TIER + 2, Start, (VisitQuest,), 1203, 1303, NA, 2903, DialogDict[2902]),
    2903: (DD_TIER + 2, Cont, (DeliverItemQuest, 2009), Same, 1106, NA, 2904, DialogDict[2903]),
    2904: (DD_TIER + 2, Cont, (DeliverItemQuest, 2010), Same, 1203, NA, 2905, DialogDict[2904]),
    2905: (DD_TIER + 2, Cont, (VisitQuest, 2009), Same, 1105, NA, 2906, DialogDict[2905]),
    2906: (DD_TIER + 2, Cont, (DeliverGagQuest, 6, ToontownBattleGlobals.SQUIRT_TRACK, 3), Same, Same, NA, 2907, DialogDict[2906]),
    2907: (DD_TIER + 2, Cont, (DeliverItemQuest, 2011), Same, 1203, NA, (2910, 2915, 2920), DialogDict[2907]),
    2910: (DD_TIER + 2, Cont, (VisitQuest,), Same, 1107, NA, 2911, TTLocalizer.QuestDialog_2910),
    2911: (DD_TIER + 2, Cont, (CogTrackQuest, ToontownGlobals.DonaldsDock, 4, 'm'), Same, Same, NA, 2925, DialogDict[2911]),
    2915: (DD_TIER + 2, Cont, (VisitQuest,), Same, 1204, NA, 2916, TTLocalizer.QuestDialog_2910),
    2916: (DD_TIER + 2, Cont, (CogTrackQuest, ToontownGlobals.DonaldsDock, 2, 's'), Same, Same, NA, 2925, DialogDict[2916]),
    2920: (DD_TIER + 2, Cont, (VisitQuest,), Same, 1204, NA, 2921, TTLocalizer.QuestDialog_2910),
    2921: (DD_TIER + 2, Cont, (CogTrackQuest, ToontownGlobals.DonaldsDock, 6, 'c'), Same, Same, NA, 2925, DialogDict[2921]),
    2925: (DD_TIER + 2, Cont, (DeliverItemQuest, 2012), Same, 1203, NA, 2926, DialogDict[2925]),
    2926: (DD_TIER + 2, Cont, (BuildingQuest, ToontownGlobals.DonaldsDock, 1, Any, 2), Same, Same, 900, NA, DialogDict[2926]),

    # New DD Tasks are here.
    # Tier, Start Flag, Quest Class, FromNPC, ToNPC, Reward, Next ID, Dialog
    2399: (DD_TIER + 1, Start, (VisitQuest,), Any, 1112, NA, 2400, DefaultDialog),
    2400: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.LighthouseLane, 8, 2099, Easy, AnyFish), Same, Same, NA, 2401, DialogDict[2400]),
    2401: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.BarnacleBoulevard, 1, 2100, Easy, 'c', 'track', 3), Same, Same, NA, 2402, DialogDict[2401]),
    2402: (DD_TIER + 1, Cont, (DeliverItemQuest, 2100,), 1112, 1313, NA, 2403, DialogDict[2402]),
    2403: (DD_TIER + 1, Cont, (CogQuest, ToontownGlobals.LighthouseLane, 5, Any), Same, Same, NA, 2404, DialogDict[2403]),
    2404: (DD_TIER + 1, Cont, (DeliverItemQuest, 2101,), 1313, 1112, 100, NA, DialogDict[2404]),

    2405: (DD_TIER + 1, Start, (VisitQuest,), Any, 1116, NA, 2406, DefaultDialog),
    2406: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 4, 2102, Easy, 'p', 'type'), Same, Same, NA, 2407, DialogDict[2406]),
    2407: (DD_TIER + 1, Cont, (BuildingQuest, ToontownGlobals.BarnacleBoulevard, 3, Any, 1), Same, Same, 201, NA, DialogDict[2407]),

    2408: (DD_TIER + 1, Start, (VisitQuest,), Any, 1123, NA, 2409, DefaultDialog),
    2409: (DD_TIER + 1, Cont, (VisitQuest,), 1123, 1221, NA, 2410, DialogDict[2409]),
    2410: (DD_TIER + 1, Cont, (BuildingQuest, ToontownGlobals.SeaweedStreet, 1, Any, 1), Same, Same, NA, 2411, DialogDict[2410]),
    2411: (DD_TIER + 1, Cont, (DeliverItemQuest, 2103), Same, 1123, NA, 2412, DialogDict[2411]),
    2412: (DD_TIER + 1, Cont, (VisitQuest,), 1123, 1322, NA, 2413, DialogDict[2412]),
    2413: (DD_TIER + 1, Cont, (BuildingQuest, ToontownGlobals.LighthouseLane, 1, Any, 1), Same, Same, NA, 2414, DialogDict[2413]),
    2414: (DD_TIER + 1, Cont, (DeliverItemQuest, 2104), Same, 1123, NA, 2415, DialogDict[2414]),
    2415: (DD_TIER + 1, Cont, (BuildingQuest, ToontownGlobals.BarnacleBoulevard, 1, Any, 1), Same, Same, 101, NA, DialogDict[2415]),

    2416: (DD_TIER + 1, Start, (VisitQuest,), Any, 1210, NA, 2417, DefaultDialog),
    2417: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 5, 2105, 80, 'c', 'track', 4), Same, Same, NA, 2418, DialogDict[2417]),
    2418: (DD_TIER + 1, Cont, (VisitQuest,), 1210, 1223, NA, 2419, DialogDict[2418]),
    2419: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.SeaweedStreet, 1, 2106, Medium, AnyFish,), Same, 1210, NA, 2420, DialogDict[2419]),
    2420: (DD_TIER + 1, Cont, (BuildingQuest, ToontownGlobals.SeaweedStreet, 1, Any, 2), Same, Same, 100, NA, DialogDict[2420]),

    2421: (DD_TIER + 1, Start, (VisitQuest,), Any, 1219, NA, 2422, DefaultDialog),
    2422: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1226, NA, 2423, DialogDict[2422]),
    2423: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 4, 2107, Easy, 4, 'level'), Same, 1219, NA, 2425, DialogDict[2423]),
    2425: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1211, NA, (2426, 2427), DialogDict[2425]),
    2426: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 2108, Easy, 'mb', 'type'), Same, 1219, NA, 2428, DialogDict[2426]),
    2427: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 2108, Easy, 'hh', 'type'), Same, 1219, NA, 2428, DialogDict[2426]),
    2428: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1225, NA, (2429, 2430), DialogDict[2428]),
    2429: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 10, 2109, Easy, 'c', 'track', 3), Same, 1219, 102, NA, DialogDict[2429]),
    2430: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 10, 2109, Easy, 'm', 'track', 3), Same, 1219, 102, NA, DialogDict[2429]),

    2431: (DD_TIER + 1, Start, (VisitQuest,), Any, 1323, NA, 2432, DefaultDialog),
    2432: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.LighthouseLane, 12, 2110, Easy, AnyFish), Same, Same, NA, 2433, DialogDict[2432]),
    2433: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.LighthouseLane, 12, 2111, 85, 4, 'level'), Same, Same, NA, 2434, DialogDict[2433]),
    2434: (DD_TIER + 1, Cont, (DeliverItemQuest, 2112), Same, 1320, NA, 2435, DialogDict[2434]),
    2435: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1323, NA, 2436, DialogDict[2435]),
    2436: (DD_TIER + 1, Cont, (DeliverItemQuest, 2112), Same, 1321, NA, 2437, DefaultDialog),
    2437: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1323, 102, NA, DialogDict[2437]),

    2438: (DD_TIER + 1, Start, (VisitQuest,), Any, 1317, NA, (2439, 2440), DefaultDialog),
    2439: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 2113, Easy, 'gh', 'type'), Same, Same, NA, 2441, DialogDict[2439]),
    2440: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 2113, Easy, 'bc', 'type'), Same, Same, NA, 2441, DialogDict[2439]),
    2441: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1314, NA, (2442, 2443), DefaultDialog),
    2442: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 4, 2114, 90, 'ms', 'type'), Same, 1317, NA, 2444, DefaultDialog),
    2443: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 4, 2114, 90, 'nc', 'type'), Same, 1317, NA, 2444, DefaultDialog),
    2444: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1310, NA, (2445, 2446), DefaultDialog),
    2445: (DD_TIER + 1, Cont, (BuildingQuest, Anywhere, 1, 'l', 2), Same, Same, NA, 2447, DefaultDialog),
    2446: (DD_TIER + 1, Cont, (BuildingQuest, Anywhere, 1, 'c', 2), Same, Same, NA, 2447, DefaultDialog),
    2447: (DD_TIER + 1, Cont, (DeliverItemQuest, 2115), Same, 1317, 101, NA, DefaultDialog),

    2448: (DD_TIER + 1, Start, (VisitQuest,), Any, 1222, NA, 2449, DefaultDialog),
    2449: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1218, NA, (2450, 2451), DefaultDialog),
    2450: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.DonaldsDock, 1, 2116, MediumHard, 'm', 'track'), Same, Same, NA, 2452, DefaultDialog),
    2451: (DD_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.DonaldsDock, 1, 2116, MediumHard, 'l', 'track'), Same, Same, NA, 2452, DefaultDialog),
    2452: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1103, NA, (2453, 2454), DefaultDialog),
    2453: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 2117, Medium, 'bc', 'type'), Same, 1222, NA, 2455, DefaultDialog),
    2454: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 2117, Medium, 'ac', 'type'), Same, 1222, NA, 2455, DefaultDialog),
    2455: (DD_TIER + 1, Cont, (VisitQuest,), Same, 1311, NA, 2456, DefaultDialog),
    2456: (DD_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 4, 2118, Medium, 'ds', 'type'), Same, Same, NA, 2457, DefaultDialog),
    2457: (DD_TIER + 1, Cont, (DeliverItemQuest, 2119), Same, 1222, 301, NA, DefaultDialog),
    ...
    3104: (DG_TIER, Start, (CogQuest, Anywhere, 20, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    3105: (DG_TIER, Start, (CogQuest, Anywhere, 25, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    3106: (DG_TIER, Start, (CogQuest, Anywhere, 30, Any), Any, ToonHQ, Any, NA, DefaultDialog),
    ...
    3131: (DG_TIER, Start, (CogLevelQuest, Anywhere, 10, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    3132: (DG_TIER, Start, (CogLevelQuest, Anywhere, 15, 4), Any, ToonHQ, Any, NA, DefaultDialog),
    3133: (DG_TIER, Start, (CogLevelQuest, Anywhere, 8, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    3134: (DG_TIER, Start, (CogLevelQuest, Anywhere, 12, 5), Any, ToonHQ, Any, NA, DefaultDialog),
    3135: (DG_TIER, Start, (CogLevelQuest, Anywhere, 4, 6), Any, ToonHQ, Any, NA, DefaultDialog),
    3136: (DG_TIER, Start, (CogLevelQuest, Anywhere, 6, 6), Any, ToonHQ, Any, NA, DefaultDialog),
    3137: (DG_TIER, Start, (CogLevelQuest, Anywhere, 3, 7), Any, ToonHQ, Any, NA, DefaultDialog),
    3138: (DG_TIER, Start, (CogLevelQuest, Anywhere, 6, 7), Any, ToonHQ, Any, NA, DefaultDialog),
    3139: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 6, 'm'), Any, ToonHQ, OBSOLETE, NA, DefaultDialog),
    3140: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 12, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    3141: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 6, 'c'), Any, ToonHQ, OBSOLETE, NA, DefaultDialog),
    3142: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 12, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    3143: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 10, 'm'), Any, ToonHQ, OBSOLETE, NA, DefaultDialog),
    3144: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 20, 's'), Any, ToonHQ, Any, NA, DefaultDialog),
    3145: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 10, 'c'), Any, ToonHQ, OBSOLETE, NA, DefaultDialog),
    3146: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.DaisyGardens, 20, 'l'), Any, ToonHQ, Any, NA, DefaultDialog),
    ...
    3200: (DG_TIER, Start, (VisitQuest,), Any, 5101, NA, 3201, DialogDict[3200]),
    3201: (DG_TIER, Start, (DeliverItemQuest, 5001), 5101, 5206, NA, 3203, DialogDict[3201]),
    3203: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5002, Hard, 'cr'), Same, Same, 100, NA, DialogDict[3203]),

    3204: (DG_TIER, Start, (VisitQuest,), Any, 5106, NA, 3205, DialogDict[3204]),
    3205: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 4, 5003, Medium, 'ac'), Same, Same, 100, NA, DialogDict[3205]),

    3206: (DG_TIER, Start, (VisitQuest,), Any, 5107, NA, 3207, DialogDict[3206]),
    3207: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 10, 5004, 85, 'dt'), Same, Same, NA, 3208, DialogDict[3207]),
    3208: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 5, 5004, 85, 'bs'), Same, Same, NA, 3209, DialogDict[3208]),
    3209: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 3, 5004, 85, 'sd'), Same, Same, 101, NA, DialogDict[3209]),

    3210: (DG_TIER, Start, (DeliverGagQuest, 3, ToontownBattleGlobals.SQUIRT_TRACK, 4), Any, 5207, NA, 3211, DialogDict[3210]),
    3211: (DG_TIER, Cont, (CogQuest, 5200, 20, Any), Same, Same, 100, NA, DialogDict[3211]),

    3212: (DG_TIER, Start, (VisitQuest,), Any, 5208, NA, 3213, DialogDict[3212]),
    3213: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 1, 5005, 40, 'gh'), 5208, Same, NA, 3214, DialogDict[3213]),
    3214: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 1, 5006, 40, 'ms'), Same, Same, NA, 3215, DialogDict[3214]),
    3215: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 1, 5007, 40, 'tf'), Same, Same, NA, 3216, DialogDict[3215]),
    3216: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.DaisyGardens, 1, 5008, 40, 'm'), Same, Same, 202, NA, DialogDict[3216]),

    3217: (DG_TIER, Start, (RecoverItemQuest, Anywhere, 1, 5010, VeryEasy, 'nd'), ToonHQ, ToonHQ, NA, 3218, DialogDict[3217]),
    3218: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5010, Medium, 'ms'), Same, Same, NA, 3219, DialogDict[3218]),
    3219: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5010, Easy, 'm'), Same, Same, 101, NA, DialogDict[3219]),

    3244: (DG_TIER, Start, (RecoverItemQuest, Anywhere, 1, 5010, VeryEasy, 'dt'), ToonHQ, ToonHQ, NA, 3245, DialogDict[3244]),
    3245: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5010, Medium, 'bs'), Same, Same, NA, 3246, DialogDict[3245]),
    3246: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5010, Easy, 'le'), Same, Same, 101, NA, DialogDict[3246]),

    3220: (DG_TIER, Start, (VisitQuest,), Any, 5207, NA, 3210, DialogDict[3220]),
    3221: (DG_TIER, Cont, (CogQuest, ToontownGlobals.DaisyGardens, 20, Any), 5207, Same, 100, NA, DialogDict[3221]),

    3222: (DG_TIER, Start, (BuildingQuest, Anywhere, 2, Any, 1), ToonHQ, ToonHQ, NA, 3223, DialogDict[3222]),
    3223: (DG_TIER, Cont, (BuildingQuest, Anywhere, 2, Any, 2), Same, Same, NA, 3224, DialogDict[3223]),
    3224: (DG_TIER, Cont, (BuildingQuest, Anywhere, 2, Any, 3), Same, Same, 501, NA, DialogDict[3224]),
    3225: (DG_TIER, Start, (VisitQuest,), Any, 5108, NA, (3226, 3227, 3228, 3229, 3230, 3231, 3232, 3233, 3234), DialogDict[3225]),
    ...
    3235: (DG_TIER, Cont, (CogQuest, ToontownGlobals.DaisyGardens, 10, Any), Same, 5108, 100, NA, DialogDict[3235]),
    3236: (DG_TIER, Start, (BuildingQuest, Anywhere, 3, 'l', 2), Any, ToonHQ, NA, 3237, DialogDict[3236]),
    3237: (DG_TIER, Cont, (BuildingQuest, Anywhere, 3, 's', 2), Same, Same, 702, NA, DialogDict[3237]),
    3238: (DG_TIER, Start, (RecoverItemQuest, Anywhere, 1, 2, VeryEasy, 'm'), Any, ToonHQ, NA, 3239, DialogDict[3238]),
    3239: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5012, Hard, 'm'), Same, Same, 302, NA, DialogDict[3239]),
    3242: (DG_TIER, Start, (RecoverItemQuest, Anywhere, 1, 2, VeryEasy, 'le'), Any, ToonHQ, NA, 3243, DialogDict[3242]),
    3243: (DG_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 5012, Hard, 'le'), Same, Same, 302, NA, DialogDict[3243]),
    3240: (DG_TIER, Start, (RecoverItemQuest, Anywhere, 1, 5009, Hard, 'le'), Any, 5103, 102, NA, DialogDict[3240]),
    3241: (DG_TIER, Start, (BuildingQuest, Anywhere, 5, Any, 3), Any, ToonHQ, 102, NA, DialogDict[3241]),
    3250: (DG_TIER, Start, (VisitQuest,), Any, 5317, NA, 3251, DialogDict[3250]),
    3251: (DG_TIER, Start, (CogTrackQuest, ToontownGlobals.SellbotHQ, 12, 's'), 5317, Same, NA, 3252, DialogDict[3251]),
    3252: (DG_TIER, Cont, (VisitQuest,), Same, 5311, NA, 3253, DialogDict[3252]),
    3253: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.SellbotHQ, 1, 5013, Medium, 's', 'track'), Same, Same, NA, 3254, DialogDict[3253]),
    3254: (DG_TIER, Cont, (DeliverItemQuest, 5013), Same, 5317, 202, NA, DialogDict[3254]),
    3255: (DG_TIER, Start, (VisitQuest,), Any, 5314, NA, 3258, DialogDict[3255]),
    3256: (DG_TIER, Start, (VisitQuest,), Any, 5315, NA, 3258, DialogDict[3256]),
    3257: (DG_TIER, Start, (VisitQuest,), Any, 5316, NA, 3258, DialogDict[3257]),
    3258: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.SellbotHQ, 1, 5014, VeryEasy, 's', 'track'), Same, Same, NA, 3259, DialogDict[3258]),
    3259: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.SellbotHQ, 1, 5015, Easy, 's', 'track'), Same, Same, NA, 3260, DialogDict[3259]),
    3260: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.SellbotHQ, 1, 5016, Medium, 's', 'track'), Same, Same, NA, 3261, DialogDict[3260]),
    3261: (DG_TIER, Cont, (RecoverItemQuest, ToontownGlobals.SellbotHQ, 1, 5017, Hard, 's', 'track'), Same, Same, 102, NA, DialogDict[3261]),
    3262: (DG_TIER, Start, (VisitQuest,), Any, 5313, NA, 3263, DialogDict[3262]),
    3263: (DG_TIER, Start, (CogQuest, ToontownGlobals.SellbotHQ, 20, Any), 5313, 5313, 702, NA, DialogDict[3263]),
    ...
    4001: (MM_TIER, Start, (TrackChoiceQuest, ToontownBattleGlobals.TRAP_TRACK, ToontownBattleGlobals.HEAL_TRACK), Any, ToonHQ, 400, NA, DialogDict[4001]),
    4002: (MM_TIER, Start, (TrackChoiceQuest, ToontownBattleGlobals.TRAP_TRACK, ToontownBattleGlobals.SOUND_TRACK), Any, ToonHQ, 400, NA, DialogDict[4002]),
    ...
    4200: (MM_TIER + 1, Start, (VisitQuest,), Any, 4101, NA, 4201, DialogDict[4200]),
    4201: (MM_TIER + 1, Start, (VisitQuest,), 4101, 4201, NA, 4202, DialogDict[4201]),
    4202: (MM_TIER + 1, Cont, (DeliverItemQuest, 4001), Same, 4101, NA, 4203, DialogDict[4202]),
    4203: (MM_TIER + 1, Cont, (VisitQuest,), Same, 4301, NA, 4204, DialogDict[4203]),
    4204: (MM_TIER + 1, Cont, (CogQuest, ToontownGlobals.MinniesMelodyland, 10, Any), Same, Same, NA, 4205, DialogDict[4204]),
    4205: (MM_TIER + 1, Cont, (DeliverItemQuest, 4002), Same, 4101, NA, 4206, DialogDict[4205]),
    4206: (MM_TIER + 1, Cont, (VisitQuest,), Same, 4102, NA, 4207, DialogDict[4206]),
    4207: (MM_TIER + 1, Cont, (VisitQuest,), Same, 4108, NA, 4208, DialogDict[4207]),
    4208: (MM_TIER + 1, Cont, (DeliverGagQuest, 1, ToontownBattleGlobals.THROW_TRACK, 4), Same, Same, NA, 4209, DialogDict[4208]),
    4209: (MM_TIER + 1, Cont, (DeliverItemQuest, 4003), Same, 4102, NA, 4210, DialogDict[4209]),
    4210: (MM_TIER + 1, Cont, (DeliverItemQuest, 4004), Same, 4101, 203, NA, DialogDict[4210]),
    4211: (MM_TIER + 1, Start, (VisitQuest,), ToonHQ, 4103, NA, 4212, DialogDict[4211]),
    4212: (MM_TIER + 1, Start, (CogQuest, ToontownGlobals.MinniesMelodyland, 10, 'nc'), 4103, Same, NA, 4213, DialogDict[4212]),
    4213: (MM_TIER + 1, Cont, (CogTrackQuest, ToontownGlobals.MinniesMelodyland, 20, 'm'), Same, Same, NA, 4214, DialogDict[4213]),
    4214: (MM_TIER + 1, Cont, (BuildingQuest, Anywhere, 1, 'm', Any), Same, Same, 303, NA, DialogDict[4214]),
    4215: (MM_TIER + 1, Start, (VisitQuest,), Any, 4302, NA, 4216, DialogDict[4215]),
    4216: (MM_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 4005, VeryHard, 'gh'), 4302, Same, NA, 4217, DialogDict[4216]),
    4217: (MM_TIER + 1, Cont, (DeliverItemQuest, 4005), Same, 4203, NA, 4218, DialogDict[4217]),
    4218: (MM_TIER + 1, Cont, (VisitQuest,), Any, 4302, NA, 4219, DialogDict[4218]),
    4219: (MM_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 4006, VeryHard, 'gh'), Same, Same, NA, 4220, DialogDict[4219]),
    4220: (MM_TIER + 1, Cont, (DeliverItemQuest, 4006), Same, 4308, NA, 4221, DialogDict[4220]),
    4221: (MM_TIER + 1, Cont, (VisitQuest,), Any, 4302, NA, 4222, DialogDict[4221]),
    4222: (MM_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 4007, VeryHard, 'gh'), Same, Same, NA, 4223, DialogDict[4222]),
    4223: (MM_TIER + 1, Cont, (DeliverItemQuest, 4007), Same, 4202, NA, 4224, DialogDict[4223]),
    4224: (MM_TIER + 1, Cont, (VisitQuest,), Any, 4302, 703, NA, DialogDict[4224]),
    ...
    902: (MM_TIER + 2, Start, (VisitQuest,), Any, 4303, NA, 4903, DialogDict[902]),
    4903: (MM_TIER + 2, Start, (DeliverItemQuest, 4008), 4303, 4109, NA, 4904, DialogDict[4903]),
    4904: (MM_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 4009, VeryHard, AnyFish), Same, Same, NA, 4905, DialogDict[4904]),
    4905: (MM_TIER + 2, Cont, (BuildingQuest, Anywhere, 1, Any, 1), Same, Same, NA, 4906, DialogDict[4905]),
    4906: (MM_TIER + 2, Cont, (DeliverItemQuest, 4010), Same, 4303, NA, 4907, DialogDict[4906]),
    4907: (MM_TIER + 2, Cont, (VisitQuest,), Same, 4208, NA, 4908, DialogDict[4907]),
    4908: (MM_TIER + 2, Cont, (BuildingQuest, Anywhere, 1, Any, 2), Same, Same, NA, 4909, DialogDict[4908]),
    4909: (MM_TIER + 2, Cont, (BuildingQuest, Anywhere, 1, Any, 3), Same, Same, NA, 4910, DialogDict[4909]),
    4910: (MM_TIER + 2, Cont, (DeliverItemQuest, 4011), Same, 4303, 900, NA, DialogDict[4910]),
    ...
    5247: (BR_TIER, Start, (VisitQuest,), Any, 3112, NA, 5248, DialogDict[5247]),
    5248: (BR_TIER, Start, (CogLevelQuest, Anywhere, 10, 8), 3112, Same, NA, 5249, DialogDict[5248]),
    5249: (BR_TIER, Cont, (RecoverItemQuest, Anywhere, 3, 3018, VeryHard, AnyFish), Same, Same, NA, (5250, 5258, 5259, 5260), DialogDict[5249]),
    5250: (BR_TIER, Cont, (BuildingQuest, Anywhere, 2, 'l', 4), Same, Same, NA, (5001, 5002, 5003, 5004, 5005, 5006, 5007, 5008), DialogDict[5250]),
    5258: (BR_TIER, Cont, (BuildingQuest, Anywhere, 2, 'c', 4), Same, Same, NA, (5001, 5002, 5003, 5004, 5005, 5006, 5007, 5008), DialogDict[5258]),
    5259: (BR_TIER, Cont, (BuildingQuest, Anywhere, 2, 'm', 4), Same, Same, NA, (5001, 5002, 5003, 5004, 5005, 5006, 5007, 5008), DialogDict[5259]),
    5260: (BR_TIER, Cont, (BuildingQuest, Anywhere, 2, 's', 4), Same, Same, NA, (5001, 5002, 5003, 5004, 5005, 5006, 5007, 5008), DialogDict[5260]),
    ...
    5200: (BR_TIER + 1, Start, (VisitQuest,), Any, 3110, NA, (5201, 5261, 5262, 5263), DialogDict[5200]),
    5201: (BR_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 3001, VeryHard, 'hh'), 3110, Same, 100, NA, DialogDict[5201]),
    5261: (BR_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 3001, VeryHard, 'tf'), 3110, Same, 100, NA, DialogDict[5261]),
    5262: (BR_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 3001, VeryHard, 'mb'), 3110, Same, 100, NA, DialogDict[5262]),
    5263: (BR_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 3001, VeryHard, 'sd'), 3110, Same, 100, NA, DialogDict[5263]),
    5202: (BR_TIER + 1, Start, (VisitQuest,), Any, 3108, NA, 5203, DialogDict[5202]),
    5203: (BR_TIER + 1, Start, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 1, 3002, VeryHard, Any), 3108, Same, NA, 5204, DialogDict[5203]),
    5204: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3205, NA, 5205, DialogDict[5204]),
    5205: (BR_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 3, 3003, Hard, AnyFish), Same, Same, NA, 5206, DialogDict[5205]),
    5206: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3210, NA, 5207, DialogDict[5206]),
    5207: (BR_TIER + 1, Cont, (BuildingQuest, Anywhere, 5, Any, 4), Same, Same, NA, 5208, DialogDict[5207]),
    5208: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3114, NA, 5209, DialogDict[5208]),
    5209: (BR_TIER + 1, Cont, (CogLevelQuest, Anywhere, 20, 7), Same, Same, 204, NA, DialogDict[5209]),
    5210: (BR_TIER + 1, Start, (VisitQuest,), Any, 3206, NA, (5211, 5264, 5265, 5266), DialogDict[5210]),
    5211: (BR_TIER + 1, Start, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 1, 3004, Medium, 'le'), 3206, Same, NA, 5212, DialogDict[5211]),
    5264: (BR_TIER + 1, Start, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 1, 3004, Hard, 'ls'), 3206, Same, NA, 5212, DialogDict[5264]),
    5265: (BR_TIER + 1, Start, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 1, 3004, Hard, 'm'), 3206, Same, NA, 5212, DialogDict[5265]),
    5266: (BR_TIER + 1, Start, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 1, 3004, Hard, 'cr'), 3206, Same, NA, 5212, DialogDict[5266]),
    5212: (BR_TIER + 1, Cont, (DeliverItemQuest, 3004), Same, 3111, NA, 5213, DialogDict[5212]),
    5213: (BR_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 10, 3005, Hard, Any), Same, Same, NA, 5214, DialogDict[5213]),
    5214: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3119, NA, 5215, DialogDict[5214]),
    5215: (BR_TIER + 1, Cont, (CogLevelQuest, Anywhere, 10, 8), Same, Same, NA, 5216, DialogDict[5215]),
    5216: (BR_TIER + 1, Cont, (DeliverItemQuest, 3006), Same, 3206, 704, NA, DialogDict[5216]),
    5217: (BR_TIER + 1, Start, (VisitQuest,), Any, 3113, NA, 5218, DialogDict[5217]),
    5218: (BR_TIER + 1, Start, (CogQuest, Anywhere, 10, 'm'), 3113, Same, NA, 5219, DialogDict[5218]),
    5219: (BR_TIER + 1, Cont, (CogQuest, Anywhere, 10, 'cr'), Same, Same, NA, 5220, DialogDict[5219]),
    5220: (BR_TIER + 1, Cont, (CogQuest, Anywhere, 10, 'ls'), Same, Same, NA, 5221, DialogDict[5220]),
    5221: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3211, NA, 5222, DialogDict[5221]),
    5222: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 2, 3007, Hard, AnyFish), Same, Same, NA, 5223, DialogDict[5222]),
    5223: (BR_TIER + 1, Cont, (DeliverItemQuest, 3008), Same, 3113, NA, 5224, DialogDict[5223]),
    5224: (BR_TIER + 1, Cont, (CogQuest, Anywhere, 5, 'le'), Same, Same, 502, NA, DialogDict[5224]),
    5225: (BR_TIER + 1, Start, (VisitQuest,), Any, 3106, NA, 5226, DialogDict[5225]),
    5226: (BR_TIER + 1, Start, (BuildingQuest, Anywhere, 3, 'm', 4), 3106, Same, NA, 5227, DialogDict[5226]),
    5227: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3208, NA, 5228, DialogDict[5227]),
    5228: (BR_TIER + 1, Cont, (DeliverItemQuest, 3009), Same, 3207, NA, (5229, 5267, 5268, 5269), DialogDict[5228]),
    5229: (BR_TIER + 1, Cont, (CogTrackQuest, ToontownGlobals.TheBrrrgh, 8, 'm'), Same, Same, NA, 5230, DialogDict[5229]),
    5267: (BR_TIER + 1, Cont, (CogTrackQuest, ToontownGlobals.TheBrrrgh, 8, 's'), Same, Same, NA, 5230, DialogDict[5267]),
    5268: (BR_TIER + 1, Cont, (CogTrackQuest, ToontownGlobals.TheBrrrgh, 8, 'l'), Same, Same, NA, 5230, DialogDict[5268]),
    5269: (BR_TIER + 1, Cont, (CogTrackQuest, ToontownGlobals.TheBrrrgh, 8, 'c'), Same, Same, NA, (5230, 5270, 5271, 5272), DialogDict[5269]),
    5230: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3010, Hard, 'rb'), Same, Same, NA, 5231, DialogDict[5230]),
    5270: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3010, Hard, 'tbc'), Same, Same, NA, 5231, DialogDict[5270]),
    5271: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3010, Hard, 'mh'), Same, Same, NA, 5231, DialogDict[5271]),
    5272: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3010, Medium, 'bw'), Same, Same, NA, 5231, DialogDict[5272]),
    5231: (BR_TIER + 1, Cont, (DeliverItemQuest, 3010), Same, 3208, NA, 5232, DialogDict[5231]),
    5232: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3106, NA, 5233, DialogDict[5232]),
    5233: (BR_TIER + 1, Cont, (DeliverItemQuest, 3011), Same, 3208, 304, NA, DialogDict[5233]),
    5243: (BR_TIER + 1, Start, (VisitQuest,), Any, 3217, NA, 5244, DialogDict[5243]),
    5244: (BR_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 2007, VeryHard, 'mm'), 3217, Same, NA, 5245, DialogDict[5244]),
    5245: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3017, Hard, AnyFish), Same, Same, NA, 5246, DialogDict[5245]),
    5246: (BR_TIER + 1, Cont, (BuildingQuest, ToontownGlobals.TheBrrrgh, 5, Any, 1), Same, Same, 101, NA, DialogDict[5246]),
    5251: (BR_TIER + 1, Start, (VisitQuest,), Any, 3134, NA, 5252, DialogDict[5251]),
    5252: (BR_TIER + 1, Start, (RecoverItemQuest, Anywhere, 1, 3019, VeryHard, Any), 3134, Same, NA, (5253, 5273, 5274, 5275), DialogDict[5252]),
    5253: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3020, VeryHard, 'cr'), Same, Same, NA, (5254, 5282, 5283, 5284), DialogDict[5253]),
    5273: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3020, VeryHard, 'm'), Same, Same, NA, (5254, 5282, 5283, 5284), DialogDict[5273]),
    5274: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3020, VeryHard, 'ls'), Same, Same, NA, (5254, 5282, 5283, 5284), DialogDict[5274]),
    5275: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3020, Hard, 'le'), Same, Same, NA, (5254, 5282, 5283, 5284), DialogDict[5275]),
    5254: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3021, VeryHard, 'mh'), Same, Same, 102, NA, DialogDict[5254]),
    5282: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3021, VeryHard, 'tbc'), Same, Same, 102, NA, DialogDict[5282]),
    5283: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3021, VeryHard, 'rb'), Same, Same, 102, NA, DialogDict[5283]),
    5284: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 1, 3021, Hard, 'bw'), Same, Same, 102, NA, DialogDict[5284]),
    5255: (BR_TIER + 1, Start, (VisitQuest,), Any, 3228, NA, (5256, 5276), DialogDict[5255]),
    5256: (BR_TIER + 1, Cont, (CogTrackQuest, Anywhere, 45, 'c'), 3228, Same, NA, (5257, 5277), DialogDict[5256]),
    5276: (BR_TIER + 1, Cont, (CogTrackQuest, Anywhere, 40, 'l'), 3228, Same, NA, (5257, 5277), DialogDict[5276]),
    5257: (BR_TIER + 1, Cont, (CogTrackQuest, Anywhere, 45, 's'), Same, Same, 100, NA, DialogDict[5257]),
    5277: (BR_TIER + 1, Cont, (CogTrackQuest, Anywhere, 45, 'm'), Same, Same, 100, NA, DialogDict[5277]),
    5301: (BR_TIER + 1, Start, (VisitQuest,), Any, 3304, NA, 5302, DialogDict[5301]),
    5302: (BR_TIER + 1, Cont, (CogTrackQuest, Anywhere, 90, 'l'), Same, Same, 100, NA, DialogDict[5302]),
    5303: (BR_TIER + 1, Start, (VisitQuest,), Any, 3318, NA, 5304, DialogDict[5303]),
    5304: (BR_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 1, 3024, VeryHard, 'l', 'track'), Same, Same, NA, 5305, DialogDict[5304]),
    5305: (BR_TIER + 1, Cont, (CogLevelQuest, Anywhere, 20, 7), Same, Same, NA, 5306, DialogDict[5305]),
    5306: (BR_TIER + 1, Cont, (RecoverItemQuest, ToontownGlobals.TheBrrrgh, 2, 3025, Hard, AnyFish), Same, Same, NA, 5307, DialogDict[5306]),
    5307: (BR_TIER + 1, Cont, (BuildingQuest, Anywhere, 5, Any, 4), Same, Same, 204, NA, DialogDict[5307]),
    5308: (BR_TIER + 1, Start, (VisitQuest,), Any, 3312, NA, 5309, DialogDict[5308]),
    5309: (BR_TIER + 1, Cont, (CogTrackQuest, ToontownGlobals.PolarPlace, 30, 'l'), Same, Same, NA, 5310, DialogDict[5309]),
    5310: (BR_TIER + 1, Cont, (VisitQuest,), Same, 3113, NA, 5311, DialogDict[5310]),
    5311: (BR_TIER + 1, Cont, (RecoverItemQuest, Anywhere, 2, 3026, Medium, 'le'), Same, Same, NA, 5312, DialogDict[5311]),
    5312: (BR_TIER + 1, Cont, (DeliverItemQuest, 3026), Same, 3312, 502, NA, DialogDict[5312]),
    ...
    903: (BR_TIER + 2, Start, (VisitQuest,), Any, 3112, NA, (5234, 5278), DialogDict[903]),
    5234: (BR_TIER + 2, Start, (RecoverItemQuest, Anywhere, 6, 3012, Medium, 'tbc'), 3112, Same, NA, (5235, 5279), DialogDict[5234]),
    5278: (BR_TIER + 2, Start, (RecoverItemQuest, Anywhere, 6, 3022, Medium, 'mh'), 3112, Same, NA, (5235, 5279), DialogDict[5278]),
    5235: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 3013, Hard, 'rb'), Same, Same, NA, 5236, DialogDict[5235]),
    5279: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 3013, Medium, 'bw'), Same, Same, NA, 5236, DialogDict[5279]),
    5236: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 3014, VeryHard, AnyFish), Same, Same, NA, 5237, DialogDict[5236]),
    5237: (BR_TIER + 2, Cont, (VisitQuest,), Same, 3128, NA, (5238, 5280), DialogDict[5237]),
    5238: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 10, 3015, VeryEasy, 'mh'), Same, Same, NA, 5239, DialogDict[5238]),
    5280: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 10, 3015, VeryEasy, 'tbc'), Same, Same, NA, 5239, DialogDict[5280]),
    5239: (BR_TIER + 2, Cont, (DeliverItemQuest, 3015), Same, 3112, NA, (5240, 5281), DialogDict[5239]),
    5240: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 3016, Hard, 'bw'), Same, Same, NA, 5241, DialogDict[5240]),
    5281: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 3023, Hard, 'mh'), Same, Same, NA, 5241, DialogDict[5281]),
    5241: (BR_TIER + 2, Cont, (BuildingQuest, Anywhere, 20, Any, 4), Same, Same, NA, 5242, DialogDict[5241]),
    5242: (BR_TIER + 2, Cont, (RecoverItemQuest, Anywhere, 1, 3014, VeryHard, AnyFish), Same, Same, 900, NA, DialogDict[5242]),
    ...
    6201: (DL_TIER, Start, (VisitQuest,), Any, 9111, NA, 6202, DialogDict[6201]),
    6202: (DL_TIER, Start, (CogQuest, ToontownGlobals.DonaldsDreamland, 70, Any), 9111, Same, 100, NA, DialogDict[6202]),
    6206: (DL_TIER, Start, (VisitQuest,), Any, 9131, NA, 6207, DialogDict[6206]),
    6207: (DL_TIER, Start, (BuildingQuest, ToontownGlobals.DonaldsDreamland, 8, Any, 4), 9131, Same, 205, NA, DialogDict[6207]),
    6211: (DL_TIER, Start, (VisitQuest,), Any, 9217, NA, 6212, DialogDict[6211]),
    6212: (DL_TIER, Start, (RecoverItemQuest, Anywhere, 3, 6002, Medium, 'bc'), 9217, Same, NA, 6213, DialogDict[6212]),
    6213: (DL_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 6003, Hard, 'mb'), Same, Same, NA, 6214, DialogDict[6213]),
    6214: (DL_TIER, Cont, (RecoverItemQuest, Anywhere, 1, 6004, VeryHard, 'pp'), Same, Same, 101, NA, DialogDict[6214]),
    6221: (DL_TIER, Start, (VisitQuest,), Any, 9119, NA, 6222, DialogDict[6221]),
    6222: (DL_TIER, Start, (CogTrackQuest, ToontownGlobals.DonaldsDreamland, 50, 'c'), 9119, Same, 102, NA, DialogDict[6222]),
    6241: (DL_TIER, Start, (VisitQuest,), Any, 9219, NA, 6242, DialogDict[6241]),
    6242: (DL_TIER, Start, (CogQuest, ToontownGlobals.DonaldsDreamland, 25, 'nc'), 9219, Same, 705, NA, DialogDict[6242]),
    6251: (DL_TIER, Start, (VisitQuest,), Any, 9221, NA, 6252, DialogDict[6251]),
    6252: (DL_TIER, Start, (DeliverItemQuest, 6006), 9221, 9222, NA, 6253, DialogDict[6252]),
    6253: (DL_TIER, Cont, (VisitQuest,), Same, 9221, NA, 6254, DialogDict[6253]),
    6254: (DL_TIER, Cont, (DeliverItemQuest, 6007), Same, 9210, NA, 6255, DialogDict[6254]),
    6255: (DL_TIER, Cont, (CogTrackQuest, Anywhere, 70, 'm'), Same, Same, NA, 6256, DialogDict[6255]),
    6256: (DL_TIER, Cont, (VisitQuest,), Same, 9221, NA, 6257, DialogDict[6256]),
    6257: (DL_TIER, Cont, (DeliverItemQuest, 6008), Same, 9205, NA, 6258, DialogDict[6257]),
    6258: (DL_TIER, Cont, (CogQuest, Anywhere, 25, 'ms'), Same, Same, NA, 6259, DialogDict[6258]),
    6259: (DL_TIER, Cont, (VisitQuest,), Same, 9221, NA, 6260, DialogDict[6259]),
    6260: (DL_TIER, Cont, (DeliverItemQuest, 6009), Same, 9229, NA, 6261, DialogDict[6260]),
    6261: (DL_TIER, Cont, (VisitQuest,), Same, 9221, NA, 6262, DialogDict[6261]),
    6262: (DL_TIER, Cont, (DeliverItemQuest, 6010), Same, 9126, NA, 6263, DialogDict[6262]),
    6263: (DL_TIER, Cont, (DeliverItemQuest, 6010), Same, 9112, NA, 6264, DialogDict[6263]),
    6264: (DL_TIER, Cont, (DeliverItemQuest, 6011), Same, 9221, NA, 6265, DialogDict[6264]),
    6265: (DL_TIER, Cont, (DeliverItemQuest, 6012), Same, 9115, NA, 6266, DialogDict[6265]),
    6266: (DL_TIER, Cont, (VisitQuest,), Same, 9221, 103, NA, DialogDict[6266]),
    6271: (DL_TIER, Start, (VisitQuest,), Any, 9208, NA, 6272, DialogDict[6271]),
    6272: (DL_TIER, Start, (BuildingQuest, ToontownGlobals.DonaldsDreamland, 2, 'm', 5), 9208, Same, 305, NA, DialogDict[6272]),
...
RequiredRewardTrackDict = {
    TT_TIER: (100,),
    TT_TIER + 1: (400,),
    TT_TIER + 2: (100, 801, 200, 802, 803, 101, 804, 805, 102, 806, 807, 100, 808, 809, 101, 810, 811, 812, 813, 814, 815, 500, 300),
    TT_TIER + 3: (900,),
    DD_TIER: (400,),
...
def assertAllQuestsValid():
    ...
    for questId in list(QuestDict.keys()):
        ...
        if nextQuest == NA and reward == NA:
            raise InvalidQuest('Invalid Quest with no reward or next quest for %s' % questId)
