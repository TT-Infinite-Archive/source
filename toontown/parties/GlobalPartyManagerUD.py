...
class GlobalPartyManagerUD(DistributedObjectGlobalUD):
    ...
    def checkForDeletion(self, partyId, refund=False):
        ...
        if not party:
            return
        if datetime.now(ToontownTimeZone()) > party['start'] or party['status'] == PartyStatus.Cancelled:
            hostId = party['hostId']
            if refund:
    ...
    def isTooLate(self, party):
        ...
        if type(party['start']) == str:
            try:
                party['start'] = datetime.strptime(
                            party['start'].split('-04:00')[0], LOAD_TIME_FORMAT).replace(tzinfo=ToontownTimeZone())
            except:
                return True

        if type(party['end']) == str:
            try:
                party['end'] = datetime.strptime(
                            party['end'].split('-04:00')[0], LOAD_TIME_FORMAT).replace(tzinfo=ToontownTimeZone())
            except:
                return True
    ...
    def sendPartyInfo(self, sender, channel):
        for partyId, party in self.party2PubInfo.items():
            actIds = []
            for activity in self.id2Party[partyId]['activities']:
                actIds.append(activity[0])  # First part of activity tuple should be actId
            minLeft = int((PARTY_DURATION - (datetime.now(ToontownTimeZone()) - party['started']).seconds) / 60)
