...
from toontown.pets.PetFlee import PetFlee
from toontown.pets.PetWander import PetWander
from toontown.pets.PetLeash import PetLeash
...
class DistributedPetAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI, PetBase.PetBase):
    ...
    def createSphereImpulse(self):
        petRadius = 1.75
        collTrav = self.getCollTrav(name=self.doId)
        if collTrav is None:
    ...
    def __lockPetMoveTask(self, avId):
        if not hasattr(self, 'air') or avId not in self.air.doId2do:
            ...
            return Task.done
        self.lockMoveCount += 1
        av = self.air.doId2do.get(avId)
        dist = av.getDistance(self)

        # Pet's Z is calculated on the client so it shouldn't be used to get the distance.
        realDist = (av.getPos().getXy() - self.getPos().getXy()).length()
        ...
        legSize = av.getStyle().getLegSize()
        minDistance = 1.3
        if legSize == 'short':
            minDistance += .46
        self.notify.debug('avgDist: %s, realDist: %s, movieDist: %s, dist: %s,' % (avgDist, realDist, movieDist, dist))
        if realDist - movieDist > minDistance or abs(avgDist - dist) > 0.15:
            self.lockMover.move()
            if self.lockMoveCount > 3750 and dist < 3.6:
                self.lockMoveCount = 0
                return Task.done
            taskMgr.doMethodLater(simbase.petMovePeriod,
                                  self.__lockPetMoveTask, self.getLockMoveTaskName(), [avId])
        else:
