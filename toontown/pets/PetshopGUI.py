...
class PetshopGUI(DirectObject):
    ...
    class NamePicker(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, dna, traitSeed, gender):
    ...
    class AdoptPetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, petNameIndex, dna, traitSeed):
            zoneId = ZoneUtil.getCanonicalSafeZoneId(base.localAvatar.getZoneId())
            name, _, _ = PetUtil.getPetInfoFromSeed(petSeed, zoneId)
            ...
            modelPos = (0, 0, -0.3)
            modelScale = 0.055
            DirectFrame.__init__(self, relief=None, state='normal', geom=model,
                                 geom_color=ToontownGlobals.GlobalDialogColor, geom_scale=modelScale,
                                 frameSize=(-1, 1, -1, 1), pos=modelPos,
                                 text=TTLocalizer.PetshopAdoptConfirm % (name, cost), text_wordwrap=12,
                                 text_scale=0.05, text_pos=(0, 0.55), text_fg=text0Color)
    ...
    class ReturnPetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent):
            ...
            def showDialog(avatar):
                ...
                self.petModel = Pet.Pet(forGui=1)
                self.petModel.setDNA(base.localAvatar.petDNA)
                self.petModel.fitAndCenterHead(0.395, forGui=1)
                self.petModel.reparentTo(self.petView)
                self.petModel.setH(130)
    ...
    class ChoosePetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeeds, petDNA, petTraitSeeds):
            model = loader.loadModel('phase_4/models/gui/PetShopInterface')
            modelPos = (0, 0, -0.9)
            ...
            self.cancelButton = DirectButton(parent=self, relief=None, image=cancelImageList,
                                             geom=cancelIcon, scale=modelScale, pressEffect=False,
                                             command=lambda : messenger.send(doneEvent, [-1]))
            self.pawLButton = DirectButton(parent=self, relief=None, image=pawLImageList, geom=pawLArrowImageList,
                                           scale=modelScale, pressEffect=False,
                                           command=lambda : self.__handlePetChange(-1))
            self.pawRButton = DirectButton(parent=self, relief=None, image=pawRImageList,
                                           geom=pawRArrowImageList, scale=modelScale, pressEffect=False,
                                           command=lambda : self.__handlePetChange(1))
            self.okButton = DirectButton(parent=self, relief=None, image=adoptImageList,
                                         image3_color=disabledImageColor, scale=modelScale,
                                         text=TTLocalizer.PetshopAdopt, text_scale=TTLocalizer.PGUIokButton,
                                         text_pos=TTLocalizer.PGUIokButtonPos, text0_fg=text0Color,
                                         text1_fg=text1Color, text2_fg=text2Color, text3_fg=text3Color,
                                         pressEffect=False, command=lambda : messenger.send(doneEvent, [self.curPet]))
            self.moneyDisplay = DirectLabel(parent=self, relief=None,
                                            text=str(base.localAvatar.getTotalMoney()),
                                            text_scale=0.1, text_fg=(0.95, 0.95, 0, 1),
                                            text_shadow=(0, 0, 0, 1), text_pos=(0.34, 0.12),
                                            text_font=ToontownGlobals.getSignFont())

            self.saleText = DirectLabel(parent=self, relief=None,
                                            text='', text_scale=0.1, text_fg=(1, 0, 0, 1),
            ...
            self.petData = [petDNA, petTraitSeeds]
            self.makePetList()
            self.showPet()

            if self.isOnSale:
                self.saleTextInterval = Sequence(
                    LerpScaleInterval(self.saleText, duration=2.0, scale=0.7, startScale=0.5),
                    LerpScaleInterval(self.saleText, duration=2.0, scale=0.5, startScale=0.7),
                )
                self.saleTextInterval.loop()
        ...
        def makePetList(self):
            ...
            for i in range(self.numPets):
                ...
                zoneId = ZoneUtil.getCanonicalSafeZoneId(base.localAvatar.getZoneId())
                name = PetUtil.getPetInfoFromSeed(self.petSeeds[i], zoneId)[0]
                numGenders = len(PetDNA.PetGenders)
                dna = self.petData[0][i / numGenders]
                traitSeed = self.petData[1][i / numGenders]
                cost = PetUtil.getPetCostFromSeed(self.petSeeds[i], zoneId)
                cost = int(cost * self.saleMultiplier)
        ...
        def destroy(self):
            if self.isOnSale:
                if self.saleTextInterval.isPlaying():
                    self.saleTextInterval.finish()
                self.saleTextInterval = None
            self.ignore(localAvatar.uniqueName('moneyChange'))
            self.ignore(localAvatar.uniqueName('bankMoneyChange'))
            self.petModel.delete()
            DirectFrame.destroy(self)
    ...
    def __init__(self, eventDict, petSeeds, petDNA, petTraitSeeds):
        ...
        self.petSeeds = petSeeds
        self.petDNA = petDNA
        self.petTraitSeeds = petTraitSeeds
    ...
    def doDialog(self, nDialog):
        ...
        if nDialog == Dialog_MainMenu:
            ...
            self.dialog = self.AdoptPetDlg(self.adoptPetDoneEvent, self.petSeeds[self.adoptPetNum],
                                           self.adoptPetNameIndex, self.petDNA[self.adoptPetNum / 2],
                                           self.petTraitSeeds[self.adoptPetNum / 2])
        elif nDialog == Dialog_ChoosePet:
            self.acceptOnce(self.petChooserDoneEvent, self.__handleChoosePetDlg)
            self.dialog = self.ChoosePetDlg(self.petChooserDoneEvent, self.petSeeds, self.petDNA, self.petTraitSeeds)
            ...
            self.dialog = self.NamePicker(self.namePickerDoneEvent, self.petSeeds[self.adoptPetNum],
                                          self.petDNA[self.adoptPetNum / 2], self.petTraitSeeds[self.adoptPetNum / 2],
