...
class PetshopGUI(DirectObject):
    ...
    class NamePicker(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeed, gender):
            ...
            ############################################
            # come up with a list of names and a list of letters
            ...
            ###########################################
            # make our scrolling lists
            self.alphabetList = self.makeScrollList(
                self.gui, (-0.012, 0, -0.075), (1, 0.80, .80, 1),
                self.letters,
                self.makeLabel,
        ...
        def rebuildNameList(self):
            ...
            self.nameList = self.makeScrollList(
                self.gui, (0.277, 0, -0.075), (1, 0.80, .80, 1),
                self.curNames,
                self.makeLabel,
    ...
    class ChoosePetDlg(DirectFrame):
        ...
        def __init__(self, doneEvent, petSeeds):
            ...
            self.okButton = DirectButton(
                ...
                scale=modelScale,
                text=TTLocalizer.PetshopAdopt,
                text_scale=TTLocalizer.PGUIpetshopAdopt,
                text_pos=TTLocalizer.PGUIpetshopAdoptPos,
                text0_fg=text0Color,
                text1_fg=text1Color,
                text2_fg=text2Color,
                text3_fg=text3Color,
    ...
    def popDialog(self):
        self.dialogStack.pop()  # get rid of the current dialog
    ...
    def __handleMainMenuDlg(self, exitVal):
        ...
        elif exitVal == 2:  # Adopt Pet
            self.doDialog(Dialog_ChoosePet)
        elif exitVal == 3:  # Return Pet
    ...
    def __handleReturnPetDlg(self, exitVal):
        if exitVal == 0:  # Cancel
            ...
            if self.dialogStack[len(self.dialogStack) - 2] == Dialog_NamePicker:
                # the dialog before this one was the NamePicker
                # we are adopting a new pet...
                # return the old one when the new one is actually adopted
                self.doDialog(Dialog_AdoptPet)
            else:
                self.destroyDialog()
