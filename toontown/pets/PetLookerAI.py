...
class PetLookerAI:
    ...
    def _handleZoneChange(self, newZoneId, oldZoneId):
        ...
        if oldZoneData.hasCollTrav(name=self.doId):
            oldZoneData.getCollTrav(name=self.doId).removeCollider(self.lookSphereNodePath)
        oldZoneData.destroy()
        newZoneData = AIZoneData(self.air, self.parentId, newZoneId)
        if newZoneData.hasCollTrav(name=self.doId):
            newZoneData.getCollTrav(name=self.doId).addCollider(self.lookSphereNodePath, self._cHandler)
        newZoneData.destroy()
