from panda3d.core import BitMask32, CollideMask, CollisionHandler, CollisionHandlerQueue, CollisionNode, CollisionSegment, CollisionTraverser, CollisionTube, NodePath, Point3, Vec3, deg2Rad
...
class DistributedCashbotBossGoonAI(DistributedGoonAI.DistributedGoonAI, DistributedCashbotBossObjectAI.DistributedCashbotBossObjectAI):
    ...
    def requestStunned(self, pauseTime):
        ...
        if not self.hasDroppedTreasure:
            self.boss.makeTreasure(self)
            self.hasDroppedTreasure = True
