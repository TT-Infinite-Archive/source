...
from direct.interval.IntervalGlobal import Func, Parallel, ParallelEndTogether, Sequence, Track, Wait
from direct.interval.IntervalGlobal import ActorInterval, LerpFunctionInterval, ParticleInterval, SoundInterval
...
class BossCog(Avatar.Avatar):
    ...
    def generateBossCog(self):
        ...
        self.backAttack = self.rotateNode.attachNewNode('backAttack')
        self.backAttack.setPos(0, 10, 10)
        self.backAttack.setScale(2)
        self.leftAttack = self.rotateNode.attachNewNode('leftAttack')
        self.leftAttack.setPos(-10, 0, 10)
        self.leftAttack.setScale(2)
        self.rightAttack = self.rotateNode.attachNewNode('rightAttack')
    ...
    def getAnim(self, anim):
        ...
        if anim == None:
            ...
            peB = BattleParticles.loadParticleFile('bossCogFrontAttackB.ptf')
            peC = BattleParticles.loadParticleFile('bossCogFrontAttackC.ptf')
            peD = BattleParticles.loadParticleFile('bossCogFrontAttackD.ptf')
            ival = ActorInterval(self, 'Bb2Ff_spin')
            if self.forward:
                ival = Sequence(
                    Func(self.reverseBody),
                    ParallelEndTogether(ival, self.pelvis.hprInterval(0.5, self.pelvisForwardHpr, blendType='easeInOut'))
                )
            ...
            ival = Sequence(
                Track(
                    ...
                    (0.9, Parallel(
                        ...
                        ParticleInterval(peB, self.backAttack, worldRelative=0, duration=1.5, cleanup=True),
                        ParticleInterval(peC, self.leftAttack, worldRelative=0, duration=1.5, cleanup=True),
                        ParticleInterval(peD, self.rightAttack, worldRelative=0, duration=1.5, cleanup=True), duration=0)),
                    (1.9, Parallel(
                        Func(self.bubbleF.unstash),
                        Func(self.bubbleFB.unstash),
                        Func(self.bubbleFL.unstash),
                        Func(self.bubbleFR.unstash)))
                ),
                Parallel(
                    Func(self.bubbleF.stash),
                    Func(self.bubbleFB.stash),
