...
from toontown.toonbase.ToontownGlobals import SellbotFactoryInt
...
class DistributedGoonAI(DistributedCrushableEntityAI.DistributedCrushableEntityAI):
    ...
    def doCrush(self, crusherId, axis):
        ...
        if self.inSellbotFactory():
            if self.level.questManager:
                self.level.questManager.incrementQuestProgress(FQSabotageId)
    ...
    def inSellbotFactory(self):
        return self.level.getFactoryId() == SellbotFactoryInt
