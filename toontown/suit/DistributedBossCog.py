...
from direct.interval.IntervalGlobal import Sequence, Wait, Func, Parallel, ParallelEndTogether
from direct.interval.IntervalGlobal import ActorInterval, LerpFunctionInterval
...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def announceGenerate(self):
        ...
        # Gear Attack Back
        bubbleFB = CollisionSphere(0, 25, 0, 12)
        bubbleFB.setTangible(0)
        bubbleFBNode = CollisionNode('BossZap')
        bubbleFBNode.setCollideMask(ToontownGlobals.WallBitmask)
        bubbleFBNode.addSolid(bubbleFB)
        self.bubbleFB = self.rotateNode.attachNewNode(bubbleFBNode)
        self.bubbleFB.setTag('attackCode', str(ToontownGlobals.BossCogFrontAttack))
        self.bubbleFB.stash()
        ...
        if toon:
            ...
            if attackCode == ToontownGlobals.BossCogSlowDirectedAttack:
                seq = Sequence(
                    neutral1Anim,
                    Parallel(
                        ActorInterval(self, neutral, loop=1),
                        Func(self.startLookAtToon, avId)
                    ),
                    ActorInterval(self, neutral),
                    Func(self.stopLookAtToon),
                    Wait(0.01),
                    Parallel(
                        Sequence(
                            Wait(0.25),
                            Func(self.makeGearTrack, avId, attackCode),
    ...
    def startLookAtToon(self, avId):
        neutral = 'Fb_neutral'
        if not self.twoFaced:
            neutral = 'Ff_neutral'
        self.loop(neutral)
        taskMgr.add(self.lookAtToon, self.uniqueName('lookAtToon'), appendTask=False, extraArgs=[avId])

    def lookAtToon(self, avId):
        av = base.cr.doId2do.get(avId)
        if not av:
            return Task.done

        gearRoot = self.rotateNode.attachNewNode('tempGearRoot')
        gearRoot.setZ(10)
        gearRoot.headsUp(av)
        ...
        if self.turnTrack:
            if self.turnTrack.isPlaying():
                self.turnTrack.finish()

        self.turnTrack = Sequence(self.pelvis.hprInterval(0.15, VBase3(toToonH, 0, 0), blendType='easeInOut', fluid=1))
        self.turnTrack.start()
        taskMgr.doMethodLater(0.15, self.lookAtToon, self.uniqueName('lookAtToon'), appendTask=False, extraArgs=[avId])

    def stopLookAtToon(self):
        taskMgr.remove(self.uniqueName('lookAtToon'))
        if self.turnTrack:
            if self.turnTrack.isPlaying():
        ...
        if self.gearTrack:
            if self.gearTrack.isPlaying():
                self.gearTrack.finish()
                self.gearTrack = None

    def makeGearTrack(self, avId, attackCode):
        toon = base.cr.doId2do.get(avId)
        if not toon:
            return
        distance = toon.getDistance(self)
        ...
        self.gearTrack = gearTrack
        self.gearTrack.start()