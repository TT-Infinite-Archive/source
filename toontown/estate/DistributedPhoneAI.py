...
class DistributedPhoneAI(DistributedFurnitureItemAI):
    ...
    def requestGiftPurchaseMessage(self, context, toId, item, optional):
        avId = self.air.getAvatarIdFromSender()
        if avId != self.avId:
            self.air.writeServerEvent('suspicious', avId, 'Tried to purchase while not using the phone!')
            return
        ...
        self.air.deliveryManager.sendUpdate('receiveRequestPurchaseGift',
                                            [item.getBlob(), toId, avId, self.doId, context])
