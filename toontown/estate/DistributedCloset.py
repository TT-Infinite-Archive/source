...
class DistributedCloset(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def enterOpen(self):
        if self.ownerId:
            ...
            if self.customerId == base.localAvatar.doId:
                ...
                base.camera.posQuatInterval(1, (-7.58, -6.02, 6.9), (286.3, 336.8, 0), other=self, blendType='easeOut').start()
                base.camera.setPosHpr(self, -7.58, -6.02, 6.9, 286.3, 336.8, 0)
            if self.av:
                if self.avMoveTrack:
    ...
    def setMovie(self, mode, avId, timestamp):
        ...
        if mode == ClosetGlobals.CLOSET_MOVIE_CLEAR:
            ...
            if self.lastAvId == avId:
                av = base.cr.doId2do.get(avId, None)
                if av:
                    av.restoreGoofyEffect(0.5)
            if self.isLocalToon:
                self._revertGender()
                self.printInfo()
                self.resetCloset()
