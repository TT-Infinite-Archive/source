...
class DistributedGardenPlotAI(DistributedLawnDecorAI.DistributedLawnDecorAI):
    ...
    def plantNothing(self, burntBeans):
        ...
        self.notify.info(f'burning money {burntBeans}')
    ...
    def plantGagTree(self, gagTrack, gagLevel):
        ...
        if estateOwnerDoId:
            ...
            if estate:
                ...
                allOk = True
                # does the toon have an extra gag/pink slip of this type?
                if gagTrack == 9:
                    # Fire track
                    if toon.getPinkSlips() <= 0:
                        self.notify.warning(f"Toon tried to plant a fire without having a pink slip! {toon.doId}")
                        allOk = False
                else:
