...
class DistributedHouseAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def d_setDeletedItems(self, items):
        if hasattr(items, 'getBlob'):
            blob = items.getBlob(store = CatalogItem.Customization | CatalogItem.DeliveryDate)
        else:
    ...
    def setDeletedItems(self, items):
        ...
        if corrupted:
            # HACK: John's old code didn't add an expiry date on trashed items,
            # let's set them up.
            self.notify.warning(f'Deleted items {self.deletedItems} on house {self.doId} are corrupted.  Checking for no expiry date')
            oldDeletedItems = CatalogItemList.CatalogItemList(items, store = CatalogItem.Customization)
            corrupted = False
            for item in oldDeletedItems:
                if isinstance(item, CatalogInvalidItem.CatalogInvalidItem):
                    ...
                    break
            if not corrupted:
                self.notify.warning('Yup, adding date to all items...')
                # Start off with a blank list
                self.deletedItems = CatalogItemList.CatalogItemList()
                for item in oldDeletedItems:
                    # Add each item
                    self.addToDeleted(item)
                self.notify.warning('Done.')
            else:
                self.notify.warning('Nope, Deleted items corrupted big time!  Starting off fresh.')
                self.b_setDeletedItems(CatalogItemList.CatalogItemList())
    ...
    def makeRoomFor(self, count):
        ...
        if extracted:
            ...
            self.deletedItems = CatalogItemList.CatalogItemList(remaining, store = CatalogItem.Customization | CatalogItem.DeliveryDate)
            self.d_setDeletedItems(self.deletedItems)
    ...
    def checkOwner(self):
        ...
        if owner and hasattr(owner, "onOrder"):
            ...
            self.air.dbInterface.queryObject(self.air.dbId, self.ownerId, self.__gotOwnerAv, self.air.dclassesByName['DistributedToonAI'],
                                            fieldNames = ('setDeliverySchedule', 'setMailboxContents', 'setMaxHp', 'setAwardMailboxContents'))

    def __gotOwnerAv(self, dclass, fields):
        assert(self.notify.debug("__gotOwnerAv(%s, %s): %s" % (dclass, fields, self.doId)))
        if (dclass, fields) == (None, None):
            self.notify.warning("Avatar %s for house %s does not exist!" % (self.ownerId, self.doId))
            return

        if dclass != self.air.dclassesByName['DistributedToonAI']:
            self.notify.warning(f'Avatar {self.ownerId} for house {self.doId} is not a toon object! {dclass}')
            return
