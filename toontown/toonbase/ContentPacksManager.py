...
class ContentPacksManager:
    ...
    def applyMultifile(self, filename):
        ...
        """
        mf = Multifile()
        mf.openReadWrite(Filename(os.path.join(self.path, filename)))
    ...
    def readSortConfig(self):
        ...
        """
        if os.path.exists(os.path.join(self.path, 'sort.yaml')):
            with open(os.path.join(self.path, 'sort.yaml'), 'r') as f:
    ...
    def writeSortConfig(self):
        ...
        """
        with open(os.path.join(self.path, 'sort.yaml'), 'w') as f:
            for filename in self.sort:
                f.write('- %s\n' % filename)
