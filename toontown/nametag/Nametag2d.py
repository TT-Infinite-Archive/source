...
class Nametag2d(Nametag, Clickable2d, MarginVisible):
    ...
    def updateClickRegion(self):
        if self.chatBalloon is not None:
            ...
            guildText = self.getGuildText()
            if guildText and (
                    self.guildTextNode.getWidth() > self.textNode.getWidth()):
                centerX = (self.guildTextNode.getLeft() +
                           self.guildTextNode.getRight()) / 2.0
            else:
                centerX = (self.textNode.getLeft() +
                           self.textNode.getRight()) / 2.0
            bottom = self.textNode.getBottom()
            top = self.textNode.getTop()
            if guildText:
                bottom -= self.guildTextNode.getBottom()
                top += self.guildTextNode.getTop()
    ...
    def update(self):
        ...
        self.textNode.clearTransform()
        self.guildTextNode.clearTransform()
        self.chatTextNode.clearTransform()
    ...
    def drawNametag(self):
        ...
        guildText = self.getGuildText()
        if guildText:
            self.textNodePath.setZ(
                self.guildTextNode.getTop() + self.PANEL_Z_PADDING)

        # Attach the Guild TextNode:
        if guildText:
            guildTextNodePath = self.contents.attachNewNode(
                self.guildTextNode, 1)
            guildTextNodePath.setTransparency(foreground[3] < 1)
            guildTextNodePath.setAttrib(DepthWriteAttrib.make(0))
            guildTextNodePath.setY(self.TEXT_Y_OFFSET)
            guildTextNodePath.setZ(
                self.textNode.getBottom() + self.PANEL_Z_PADDING)
        ...
        # Resize the panel:
        self.panelWidth = self.PANEL_X_PADDING
        if guildText and (
                self.guildTextNode.getWidth() > self.textNode.getWidth()):
            self.panelWidth += self.guildTextNode.getWidth()
        else:
            self.panelWidth += self.textNode.getWidth()
        self.panelHeight = self.PANEL_Z_PADDING + self.textNode.getHeight()
        if guildText:
            self.panelHeight += self.guildTextNode.getHeight()
        ...
        if guildText:
            bottom += self.guildTextNode.getBottom()
        self.arrow.setZ(self.ARROW_OFFSET + bottom)
    ...
    def reposition(self):
        ...
        if self.chatBalloon is not None:
            ...
            if guildText and (
                    self.guildTextNode.getWidth() > self.textNode.getWidth()):
                left = self.guildTextNode.getLeft()
                right = self.guildTextNode.getRight()
            else:
                left = self.textNode.getLeft()
                right = self.textNode.getRight()
            bottom = self.textNode.getBottom()
            top = self.textNode.getTop()
            if guildText:
                bottom -= self.guildTextNode.getBottom()
        ...
        if self.cell in base.bottomCells:
            if self.arrow is not None:
                # Position based on the arrow instead:
                nodePath = self.arrow
                bottom = (-self.ARROW_SCALE / 2.0) + (self.PANEL_Z_PADDING * 2)
