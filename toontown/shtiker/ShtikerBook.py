...
class ShtikerBook(DirectFrame, StateData.StateData):
    ...
    def __init__(self, doneEvent):
        ...
        self.pageTabFrame = DirectFrame(parent=self, relief=None, pos=(0.93, 1, 0.645), scale=1.25)
        ...
        self.pageOrder = [
            ...
            TTLocalizer.SocialPageTitle,
            TTLocalizer.CollectiblePageTitle
         ]
    ...
    def addPageTab(self, page, pageIndex, pageName = 'Page'):
        ...
        # elif pageName == TTLocalizer.TrackPageShortTitle:
            ...
            extraArgs = [page]
        elif pageName == TTLocalizer.SocialPageTitle:
            tex = loader.loadTexture('phase_3.5/maps/toon_resistance_icon.png')
            cm = CardMaker('phase_3.5/maps/toon_resistance_icon.png card')
            cm.setFrame(-tex.getOrigFileXSize(), tex.getOrigFileXSize(), -tex.getOrigFileYSize(), tex.getOrigFileYSize())
            iconGeom = NodePath(cm.generate())
            iconGeom.setTexture(tex)
            iconGeom.setTransparency(TransparencyAttrib.MAlpha)
            iconScale = 0.006
        elif pageName == TTLocalizer.CollectiblePageTitle:
            tex = loader.loadTexture('phase_3.5/maps/collectible_tab.png')
            cm = CardMaker('phase_3.5/maps/collectible_tab.jpg card')
            cm.setFrame(-tex.getOrigFileXSize(), tex.getOrigFileXSize(), -tex.getOrigFileYSize(), tex.getOrigFileYSize())
            iconGeom = NodePath(cm.generate())
            iconGeom.setTexture(tex)
            iconGeom.setTransparency(TransparencyAttrib.MAlpha)
        ...
        pageTab = DirectButton(parent=self.pageTabFrame, relief=DGG.RAISED,
                               frameSize=(-0.575, 0.575, -0.575, 0.575),
                               borderWidth=(0.05, 0.05), text=('', '', pageName, ''),
                               text_align=TextNode.ALeft, text_pos=(1, -0.2), text_scale=TTLocalizer.SBpageTab,
                               text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), image=iconImage, image_scale=iconScale,
                               geom=iconGeom, geom_scale=iconScale, geom_color=iconColor, pos=(0, 0, -yOffset),
                               scale=0.06, command=buttonPressedCommand, extraArgs=extraArgs)
