...
from panda3d.core import ConfigVariableBool, ConfigVariableDouble, ConfigVariableInt
from direct.distributed.DistributedObjectGlobalUD import DistributedObjectGlobalUD
from direct.directnotify import DirectNotifyGlobal
from toontown.toonbase import ToontownGlobals
from toontown.parties import PartyGlobals
from toontown.parties import PartyUtils
from toontown.uberdog.ToontownPartyDatabaseUD import ToontownPartyDatabaseUD
from toontown.uberdog.ToontownInviteDatabaseUD import ToontownInviteDatabaseUD
...
class DistributedPartyManagerUD(DistributedObjectGlobalUD):
    ...
    def __init__(self, air):
        ...
        self.partyDb = ToontownPartyDatabaseUD(air)
        self.inviteDb = ToontownInviteDatabaseUD(air)

        # in minutes, how often do we check if a party can start
        self.startPartyFrequency = ConfigVariableDouble('start-party-frequency',
    ...
    def addParty(self, pmDoId, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds,
                 costOfParty):
        ...
        DistributedPartyManagerUD.notify.debug(
            "addParty( hostId=%d, startTime=%s, endTime=%s, isPrivate=%s, inviteTheme=%s, invitees=%s... )" % (
                hostId, startTime, endTime, isPrivate, PartyGlobals.EInviteTheme(inviteTheme).name,
                str(inviteeIds)))
        putSucceeded = self.partyDb.putParty(hostId, startTime, endTime, isPrivate, inviteTheme, activities,
                                             decorations, PartyGlobals.EPartyStatus.PENDING)
        if not putSucceeded:
            ...
            self.sendAddPartyResponse(pmDoId, hostId, PartyGlobals.EAddPartyErrorCode.TOO_MANY_HOSTED_PARTIES)
            return

        errorCode = PartyGlobals.EAddPartyErrorCode.ALL_OK
        partiesTuple = self.partyDb.getPartiesOfHost(hostId)
        if len(partiesTuple) > 0:
            partyId = partiesTuple[-1][
                'partyId']  # TODO-parties: is getting the -1 index guranteed to get the party we just pushed to the database?
            # send out the details of the parties he's hosting
            ...
            DistributedPartyManagerUD.notify.warning(
                "Unable to find a party for hostId %s in the party database." % hostId)
            errorCode = PartyGlobals.EAddPartyErrorCode.DATABASE_ERROR
    ...
    def markInviteAsReadButNotReplied(self, partyManagerDoId, inviteKey):
        ...
        if not invite:
            ...
            DistributedPartyManagerUD.notify.warning(
                'markInviteAsReadButNotReplied inviteKey=%s not found in inviteDb' % inviteKey)
            return
        ...
        updateResult = self.inviteDb.updateInvite(inviteKey, PartyGlobals.EInviteStatus.READ_BUT_NOT_REPLIED)
        self.updateHostAndInviteeStatus(inviteKey, partyId, invite, party, PartyGlobals.EInviteStatus.READ_BUT_NOT_REPLIED)
    ...
    def updateHostAndInviteeStatus(self, inviteKey, partyId, invite, party, newStatus):
        ...
        DistributedPartyManagerUD.notify.debug(
            "Calling DistributedToon::updateInvite( inviteKey=%s, newStatus=%s ) across the network with inviteeId %d." % (
                inviteKey, PartyGlobals.EInviteStatus(newStatus).name, inviteeId))
        ...
        DistributedPartyManagerUD.notify.debug(
            "Calling DistributedToon::updateReply( partyId=%d, inviteeId=%d, newStatus=%s ) across the network with hostId %d." % (
                partyId, inviteeId, PartyGlobals.EInviteStatus(newStatus).name, hostId))
        self.air.sendUpdateToDoId(
            "DistributedToon",
            "updateReply",
            hostId,
    ...
    def respondToInvite(self, partyManagerDoId, mailboxDoId, context, inviteKey, newStatus):
        ...
        DistributedPartyManagerUD.notify.debug(
            "respondToInvite( partyManagerDoId=%d, mailboxDoId=%d, ..., inviteKey=%d, newStatus=%s )" % (
                partyManagerDoId, mailboxDoId, inviteKey, PartyGlobals.EInviteStatus(newStatus).name))
    ...
    def _updateInvites(self, avatarId):
        ...
        DistributedPartyManagerUD.notify.debug(
            "Found %d invites for avatarId %d in the invite database." % (len(invitesTuple), avatarId))
        ...
        prioritizedPartyIds, prioritizedPartyInfo = self.reprioritizeParties(partyIds,
                                                                             PartyGlobals.MaxSetPartiesInvitedTo)
        ...
        for item in invitesTuple:
            ...
            if status == PartyGlobals.EInviteStatus.NOT_READ:
                numNew += 1
            elif status == PartyGlobals.EInviteStatus.READ_BUT_NOT_REPLIED:
                numOld += 1
    ...
    def _updateInvitedToParties(self, avatarId, passedPartyIds, passedPartyInfo):
        ...
        DistributedPartyManagerUD.notify.debug(
            "_updateInvitedToParties( avatarId=%d, partyIds=%s )" % (avatarId, partyIds))
        if partyInfo == None:
    ...
    def _updatePartyReplies(self, avatarId, hostedPartyIds, hostedParties):
        ...
        """
        DistributedPartyManagerUD.notify.debug(
            "_updatePartyReplies( avatarId=%d, hostedPartyIds=%s )" % (avatarId, hostedPartyIds))
        thresholdTime = self.getThresholdTime()
        ...
        for index, partyId in enumerate(hostedPartyIds):
            ...
            # if partyInfoDict['statusId'] != PartyGlobals.EPartyStatus.CANCELLED and \
    ...
    def changePrivateRequestAiToUd(self, pmDoId, partyId, newPrivateStatus):
        ...
        party = self.partyDb.getParty(partyId)
        if not party:
            errorCode = PartyGlobals.EChangePartyFieldErrorCode.DATABASE_ERROR
            self.air.sendUpdateToDoId(
                "DistributedPartyManager",
    ...
    def changePartyStatusRequestAiToUd(self, pmDoId, partyId, newPartyStatus):
        ...
        DistributedPartyManagerUD.notify.debug(
            "changePartyStatusRequestAiToUd partyId = %s, newPartyStatus = %s" % (partyId, newPartyStatus))
        errorCode = PartyGlobals.EChangePartyFieldErrorCode.ALL_OK
    ...
    def partyInfoOfHostRequestAiToUd(self, pmDoId, hostId):
        ...
        """
        DistributedPartyManagerUD.notify.debug(
            "partyInfoOfHostRequestAiToUd( pmDoId=%d, hostId=%d )" % (pmDoId, hostId))
        ...
        if partyFail:
            ...
            if not randomPartyCreationAllowed:
                ...
                DistributedPartyManagerUD.notify.debug(
                    "partyInfoOfHostRequestAiToUd : Creating random test party with partyId %d" % partyId)
                activities = []
                # Let's make one of each activity, arranged in a circle/oval in the party grounds
                numActivities = len(PartyGlobals.EActivityId)
    ...
    def forceFinishedForStarted(self):
        ...
        for info in result:
            ...
            if self.isOnline(hostId):
                status = PartyGlobals.EPartyStatus.FINISHED
    ...
    def forceNeverStartedForCanStart(self):
        ...
        for info in result:
            ...
            hostId = info['hostId']
            if self.isOnline(hostId):
                status = PartyGlobals.EPartyStatus.NEVER_STARTED
                self.sendNewPartyStatus(hostId, partyId, status)
    ...
    def toonHasEnteredPartyAiToUd(self, hostId):
        ...
        if hostId in self.hostAvIdToAllPartiesInfo:
            ...
            if self.hostAvIdToAllPartiesInfo[hostId][3] >= 0:
                self.sendUpdateToAllAis("updateToPublicPartyCountUdToAllAi",
    ...
    def partyHasStartedAiToUd(self, pmDoId, partyId, shardId, zoneId, hostName):
        ...
        errorCode = self.changePartyStatusRequestAiToUd(pmDoId, partyId, PartyGlobals.EPartyStatus.STARTED)
    ...
    def handleInterruptedPartiesOnShard(self, shardId):
        ...
        self.partyDb.changeMultiplePartiesStatus(interruptedPartiesToCanStart,
                                                 PartyGlobals.EPartyStatus.CAN_START)
        ...
        for index, hostId in enumerate(interruptedHostIds):
            if self.isOnline(hostId):
                ...
                if partyId in interruptedPartiesToCanStart:
                    status = PartyGlobals.EPartyStatus.CAN_START
                else:
                    status = PartyGlobals.EPartyStatus.FINISHED
                self.sendNewPartyStatus(hostId, partyId, status)
    ...
    def informInviteesPartyHasStarted(self, partyId):
        ...
        for info in inviteeDict:
            ...
            if self.isOnline(avId):
                self.sendNewPartyStatus(avId, partyId, PartyGlobals.EPartyStatus.STARTED)
