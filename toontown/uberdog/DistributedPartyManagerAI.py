...
class DistributedPartyManagerAI(DistributedObjectAI):
    ...
    def sendAddParty(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds,
        ...
        self.air.sendUpdateToDoId("DistributedPartyManager",
                                  'addParty',
                                  OtpDoGlobals.OTP_DO_ID_TOONTOWN_PARTY_MANAGER,
                                  [self.doId, hostId, startTime, endTime, isPrivate, inviteTheme, activities,
                                   decorations, inviteeIds, costOfParty])
        # Set up a failsafe incase uberdog has crashed...
        taskMgr.doMethodLater(
            5.0,
            self.addPartyResponseUdToAi,
            "NoResponseFromUberdog_%d_%d" % (self.doId, hostId),
            [hostId, PartyGlobals.EAddPartyErrorCode.DATABASE_ERROR, 0]
        )
    ...
    def addPartyRequest(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds):
        ...
        if validPartyRequest:
            validPartyRequest, costOfPartyOrError = self.validatePartyAndReturnCost(hostId, startTime, endTime,
                                                                                    isPrivate, inviteTheme, activities,
        ...
        if validPartyRequest:
            ...
            self.sendAddParty(hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations, inviteeIds,
                              costOfPartyOrError)
        else:
            DistributedPartyManagerAI.notify.debug('inValid party because : %s' % costOfPartyOrError)
            self.sendAddPartyResponse(hostId, PartyGlobals.EAddPartyErrorCode.VALIDATION_ERROR)

    def validatePartyAndReturnCost(self, hostId, startTime, endTime, isPrivate, inviteTheme, activities, decorations,
                                   inviteeIds):
        ...
        if inviteTheme not in PartyGlobals.EInviteTheme:
        ...
        if hasattr(simbase.air, "holidayManager"):
            if ToontownGlobals.VALENTINES_DAY not in simbase.air.holidayManager.currentHolidays:
                if inviteTheme == PartyGlobals.EInviteTheme.VALENTOONS:
                    return (False, "Invalid inviteTheme %s" % inviteTheme)
            if ToontownGlobals.VICTORY_PARTY_HOLIDAY not in simbase.air.holidayManager.currentHolidays:
                if inviteTheme == PartyGlobals.EInviteTheme.VICTORY_PARTY:
                    return (False, "Invalid inviteTheme %s" % inviteTheme)
        ...
        for activityTuple in activities:
            if activityTuple[0] not in PartyGlobals.EActivityId:
            ...
            if activityTuple[0] in PartyGlobals.UnreleasedActivityIds:
                self.air.writeServerEvent('suspicious', hostId, "trying to buy unreleased activity %s" %
                                          PartyGlobals.EActivityId(activityTuple[0]).name)
                self.notify.warning("%d trying to buy unreleased activity %s" %
                                    (hostId, PartyGlobals.EActivityId(activityTuple[0]).name))
        ...
        for decorationTuple in decorations:
            ...
            if decorationTuple[0] in PartyGlobals.UnreleasedDecorationIds:
                ...
                self.notify.warning("%d trying to buy unreleased decoration %s" %
                                    (hostId, PartyGlobals.EDecorationId(decorationTuple[0]).name))
                if not self.allowUnreleasedServer():
        ...
        for decorId in PartyGlobals.EDecorationId:
            if decorId in decorationsUsedDict:
                if decorationsUsedDict[decorId] > PartyGlobals.DecorationInformationDict[decorId]["limitPerParty"]:
                    return (False, "Decoration %s used too many times." % decorId)
    ...
    def respondToInviteFromMailbox(self, context, inviteKey, newStatus, mailboxDoId):
        ...
        if not toonSender:
            # the toon is not on our district, let the party manager on that district handle it
            DistributedPartyManagerAI.notify.debug(
                'respondToInviteFromMailbox toon %d is not in our district' % senderId)
            return
        self.air.sendUpdateToDoId("DistributedPartyManager",
                                  'respondToInvite',
                                  OtpDoGlobals.OTP_DO_ID_TOONTOWN_PARTY_MANAGER,
    ...
    def respondToInviteResponse(self, mailboxDoId, context, inviteKey, retcode, newStatus):
        ...
        if mailboxAI:
            if newStatus == PartyGlobals.EInviteStatus.REJECTED:
    ...
    def partyFieldChangeValidate(self, partyId):
        ...
        senderId = self.air.getAvatarIdFromSender()
        errorCode = PartyGlobals.EChangePartyFieldErrorCode.ALL_OK
        toon = simbase.air.doId2do.get(senderId)
        if not toon:
            # we don't have the toon for some reason
            errorCode = PartyGlobals.EChangePartyFieldErrorCode.VALIDATION_ERROR
            return errorCode
            ...
            for partyInfo in av.hostedParties:
                if partyInfo.partyId == partyId:
                    ...
                    if newPartyStatus == PartyGlobals.EPartyStatus.CANCELLED:
                        ...
                        self.air.writeServerEvent("party_cancel", hostId,
                                                  "%d|%d|%d|%d" % (beansRefunded, partyId, newPartyStatus, errorCode))
        self.sendUpdateToAvatarId(hostId, "changePartyStatusResponse",
    ...
    def updateToPublicPartyCountUdToAllAi(self, hostId, newCount):
        ...
        """
        DistributedPartyManagerAI.notify.debug(
            "updateToPublicPartyCountUdToAllAi : hostId=%s newCount=%s" % (hostId, newCount))
        if hostId in self.hostAvIdToAllPartiesInfo:
    ...
    def updateToPublicPartyInfoUdToAllAi(self, hostId, time, shardId, zoneId, isPrivate, numberOfGuests, hostName,
                                         activityIds, partyId):
        """
        ...
        """
        DistributedPartyManagerAI.notify.debug(
            "updateToPublicPartyInfoUdToAllAi : hostId=%s time=%s shardId=%s zoneId=%s isPrivate=%s numberOfGuests=%s hostName=%s" % (
                hostId, time, shardId, zoneId, isPrivate, numberOfGuests, hostName))
        self.hostAvIdToAllPartiesInfo[hostId] = [time, shardId, zoneId, isPrivate, numberOfGuests, hostName,
                                                 activityIds, partyId]
    ...
    def getPartyZone(self, hostId, zoneId, planningParty):
        DistributedPartyManagerAI.notify.debug(
            "getPartyZone: hostId=%s zoneId=%s planningParty=%s" % (hostId, zoneId, planningParty))
        ...
        if planningParty:
            if hostId != senderId:
                self.air.writeServerEvent('suspicious', senderId,
                                          'trying to plan party but not host : hostId = %d' % hostId)
            ...
            DistributedPartyManagerAI.notify.debug(
                "getPartyZone : Avatar %s is planning party in zone %s" % (senderId, zoneId))
        ...
        if senderId == hostId:
            ...
            if (senderId not in self.hostAvIdToPartiesRunning) and (senderId not in self.hostAvIdToClosingPartyZoneId):
        ...
        # Else sender is visiting or attending a party at this shard:
            ...
            if avPartyZoneId is None:
                ...
                try:
                    ...
                    DistributedPartyManagerAI.notify.debug(
                        "couldn't stop toonUpTask for av %s" % self.air.getAvatarIdFromSender())
                self.__sendNoPartyZoneToClient(senderId)
                return
        ...
        if senderPartyZoneId is not None:
            ...
            try:
                if senderPartyZoneId == avPartyZoneId and not isHostStartingParty:
                    DistributedPartyManagerAI.notify.debug(
                        "We are staying in the same zone %s, don't delete." % senderPartyZoneId)
                    self.__sendPartyZoneToClient(senderId, hostId)
                    return
                else:
                    DistributedPartyManagerAI.notify.debug(
                        "Sender is goint to a different party. Party av zone = %s, sender zone = %s." % (
                            avPartyZoneId, senderPartyZoneId))
            except:
        ...
        if avId == hostId:
            ...
            if avId not in self.hostAvIdToAllPartiesInfo:
                self.air.sendUpdateToDoId(
                    ...
                    OtpDoGlobals.OTP_DO_ID_TOONTOWN_PARTY_MANAGER,
                    [self.doId, self.hostAvIdToPartiesRunning[hostId].partyInfo.partyId, self.air.districtId, zoneId,
                     av.getName()]
                )
    ...
    def __sendPartyZoneToClient(self, avId, hostId):
        ...
        try:
            ...
            self.notify.warning(
                "__sendPartyZoneToClient : zone did not exist for party host %d, and visitor %d" % (hostId, avId))
    ...
    def clearPartyZoneId(self, avId, zoneIdFromClient=None):
        ...
        if removeFromDict:
            DistributedPartyManagerAI.notify.debug(
                "%s(%s):Removed %s:%s" % (defName, lineno, avId, self.avIdToPartyZoneId[avId]))
    ...
    def handleGetPartyInfo(self, partyInfo, inviteeIds):
        ...
        if partyInfo.hostId in self.hostAvIdToPartiesRunning:
            ...
            self.notify.info(
                'Finished creating party : partyId %s init, host = %s' % (partyInfo.partyId, partyInfo.hostId))
        ...
        if avZoneId is not None:
            ...
            if avZoneId in self.zoneIdToGuestAvIds:
                if avId in self.zoneIdToGuestAvIds[avZoneId]:
                    ...
                    DistributedPartyManagerAI.notify.debug(
                        "wasn't in zoneIdToGuestAvIds list: %s, %s" % (avZoneId, avId))
            else:
    ...
    def freeZoneIdFromPlannedParty(self, hostId, zoneId):
        ...
        if senderId != hostId:
            self.air.writeServerEvent('suspicious', senderId,
                                      'someone else trying to free a zone for this avatar: hostId = %d' % hostId)
            return
        if hostId in self.hostIdToPlanningPartyZoneId:
            DistributedPartyManagerAI.notify.debug(
                "freeZoneIdFromPlannedParty : freeing zone : hostId = %d, zoneId = %d" % (hostId, zoneId))
            self.air.deallocateZone(self.hostIdToPlanningPartyZoneId[hostId])
            del self.hostIdToPlanningPartyZoneId[hostId]
        else:
            self.notify.warning(
                'suspicious senderId=%d trying to free a zone that this avatar did not allocate: hostId = %d' % (
                    senderId, hostId))
            self.air.writeServerEvent('suspicious', senderId,
                                      'trying to free a zone that this avatar did not allocate: hostId = %d' % hostId)
    ...
    def __deleteParty(self, hostId):
        ...
        if hostId in self.hostAvIdToAllPartiesInfo:
            if self.hostAvIdToPartiesRunning[hostId] != None:
                ...
                DistributedPartyManagerAI.notify.debug(
                    'DistributedPartyAI requestDelete, doId=%d' % getattr(self.hostAvIdToPartiesRunning[hostId],
                                                                          'doId'))
    ...
    def __setPartyEnded(self, hostId, zoneId):
        ...
        self.air.sendUpdateToDoId(
            ...
            'changePartyStatusRequestAiToUd',
            OtpDoGlobals.OTP_DO_ID_TOONTOWN_PARTY_MANAGER,
            [self.doId, self.hostAvIdToPartiesRunning[hostId].partyInfo.partyId, PartyGlobals.EPartyStatus.FINISHED]
        )
        ...
        if guests:
            for avId in guests:
                # Pass the message to the client, who will pass it to the PartyHood
                self.sendUpdateToAvatarId(avId, "sendAvToPlayground",
                                          [avId, 0])  # 0 is a warning, 1 is final
    ...
    def partyManagerUdStartingUp(self):
        ...
        for hostId in self.hostAvIdToAllPartiesInfo:
            if hostId not in self.hostAvIdToPartiesRunning:
                self.notify.warning(
                    'hostId %d is in self.hostAvIdToAllPartiesInfo but not in self.hostAvIdToPartiesRunning' % hostId)
    ...
    def magicWordEnd(self, senderId):
        ...
        taskMgr.doMethodLater(0.1 + kickDelay + 10.0, self.__cleanupParty,
                              "DistributedPartyManagerAI_CleanUpPartyZone_%d" % partyZoneId, [hostId, partyZoneId])
