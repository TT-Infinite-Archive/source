...
class ToonHood(Hood):
    ...
    STORAGE_DNA = None
    SKY_FILE = None
    NIGHTSKY_FILE = 'phase_8/models/props/DL_sky'
    SUNSKY_FILE = 'phase_6/models/props/MM_sky'
    ...
    def handlePurchaseDone(self):
        ...
        if doneStatus is None:
            self.notify.warning('Got doneStatus of None, aborting.')
            return
    ...
    def startSky(self):
        if not self.sky.getTag('sky') == 'Regular' and not self.wantSpookySky:
            self.endSpookySky()
        elif self.wantSpookySky:
            self.startSpookySky()
        SkyUtil.startCloudSky(self)
        if self.nightSkyFile:
            SkyUtil.setupSky(self.nightSky)
        if self.sunSkyFile:
            SkyUtil.setupSky(self.sunSky)
