...
from toontown.toonbase.ToontownClientGlobals import safeZoneCountMap, townCountMap
...
class Hood(StateData.StateData):
    ...
    def doSpawnTitleText(self, text):
        ...
        self.titleTextSeq = Sequence(Wait(0.1), Wait(6.0), self.titleText.colorScaleInterval(0.5, Vec4(1.0, 1.0, 1.0, 0.0)), Func(self.titleText.hide))
    ...
    def unload(self):
        ...
        self.sky.removeNode()
        del self.sky
        if hasattr(self, 'nightSky'):
            self.nightSky.removeNode()
            del self.nightSky
        if hasattr(self, 'sunSky'):
            self.sunSky.removeNode()
            del self.sunSky
        self.ignoreAll()
    ...
    def stopSky(self):
        ...
        if self.nightSkyFile:
            self.nightSky.reparentTo(hidden)
        if self.sunSkyFile:
            self.sunSky.reparentTo(hidden)
