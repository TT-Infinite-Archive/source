...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        skipButtonImage = (self._battleGui.find('**/tt_t_gui_gen_skipSectionUp'), self._battleGui.find('**/tt_t_gui_gen_skipSectionDown'), self._battleGui.find('**/tt_t_gui_gen_skipSectionRollOver'), self._battleGui.find('**/tt_t_gui_gen_skipSectionDisabled'))
        self.skipButton = DirectButton(
            parent=self, relief=None, image=skipButtonImage, pos=(0.815, 0, -0.395), scale=(0.39, 1.0, 0.39),
            text=('', TTLocalizer.RewardPanelSkip, TTLocalizer.RewardPanelSkip, ''), text_scale=TTLocalizer.RPskipScale,
            text_fg=Vec4(1, 1, 1, 1), text_shadow=Vec4(0, 0, 0, 1), text_pos=TTLocalizer.RPskipPos, textMayChange=0,
    ...
    def getItemIntervalList(self, toon, itemList):
        ...
        for itemId in itemList:
            ...
            if itemName in items:
                items[itemName] += 1 * base.cr.progressMultiplier
            else:
                items[itemName] = 1 * base.cr.progressMultiplier
        ...
        for item in items:
            if items[item] == 1:
                recoveredText += str(items[item]) + ' ' + item + '\n'
            else:
                recoveredText += str(items[item]) + ' ' + item + 's\n'

        intervalList.append(Func(self.itemLabel.setProp, 'text', recoveredText))
        intervalList.append(Wait(len(items) + 2))
    ...
    def getMissedItemIntervalList(self, toon, missedItemList):
        ...
        for item in items:
            if items[item] == 1:
                unrecoveredText += str(items[item]) + ' ' + item + '\n'
            else:
                unrecoveredText += str(items[item]) + ' ' + item + 's\n'

        intervalList.append(Func(self.missedItemLabel.setProp, 'text', unrecoveredText))
        intervalList.append(Wait(len(items) + 2))
