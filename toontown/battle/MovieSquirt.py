...
def __getSuitTrack(suit, tContact, tDodge, hp, hpbonus, kbbonus, anim, died, leftSuits, rightSuits, battle, toon, fShowStun, beforeStun = 0.5, afterStun = 1.8, geyser = 0, uberRepeat = 0, revived = 0):
    if hp > 0:
        ...
        if kbbonus > 0:
            ...
            bonusTrack.append(Func(MovieUtil.updateHealthBar, suit, kbbonus))
        if hpbonus > 0:
            bonusTrack.append(Wait(0.75))
            bonusTrack.append(Func(suit.showHpText, -hpbonus, 1, openEnded=0, attackTrack=SQUIRT_TRACK))
            bonusTrack.append(Func(MovieUtil.updateHealthBar, suit, hpbonus))
        if died != 0:
...
def __doWaterGlass(squirt, delay, fShowStun):
    ...
    glassTrack = Sequence(Func(MovieUtil.showProp, glass, hand_jointpath0), ActorInterval(glass, 'glass'), Func(hand_jointpath1.removeNode), Func(hand_jointpath0.removeNode), Func(toon.loop, 'neutral'), Func(MovieUtil.removeProp, glass))
