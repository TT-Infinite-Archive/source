...
class CatalogScreen(DirectFrame):
    ...
    def showPageItems(self):
        ...
        if self.pageIndex < 0:
            ...
            for panel in self.panelDict[page.get_key()]:
                if not panel.load():
                    if panel in self.panelList:
                        self.panelList.remove(panel)
                    if panel in self.backPanelList:
                        self.backPanelList.remove(panel)
                    if panel in self.loyaltyPanelList:
                        self.loyaltyPanelList.remove(panel)
                    if panel in self.emblemPanelList:
                        self.emblemPanelList.remove(panel)
                    if panel in self.visiblePanels:
                        self.visiblePanels.remove(panel)
                else:
                    if panel.ival:
                        panel.ival.loop()
    ...
    def packPages(self, panelList, pageList, prefix):
        ...
        for item in panelList:
            type = item['item'].getTypeCode()
            if type == CatalogItemTypes.PET_TRICK_ITEM and not hasattr(base.localAvatar, 'petDNA'):
                continue
            if i == 0 and j == 0:
                numPages += 1
    ...
    def load(self, guiItems, guiButton, guiBack):
        ...
        self.hangup = DirectButton(base.a2dBottomRight, relief=None, pos=(-0.158, 0, 0.14), scale=(0.7, 0.7, 0.7), image=[hangupGui,
         hangupRolloverGui,
         hangupRolloverGui,
         hangupGui], text=['', TTLocalizer.CatalogHangUp, TTLocalizer.CatalogHangUp], text_fg=Vec4(1), text_scale=0.07,
                                   text_pos=(0.0, 0.14), command=self.hangUp)
        self.hangup.setBin('gui-popup', 50)
    ...
    def setClarabelleChat(self, text, timeout = 6):
        ...
        self.clarabelleChatNP = self.attachNewNode(self.clarabelleChat.node())
        ...
        if self.frienddoId and self.allowGetDetails:
            if self.giftAvatar:
                if hasattr(self.giftAvatar, 'doId'):
                    ...
                    self.giftAvatar.delete()
                self.giftAvatar = None
            self.giftAvatar = DistributedToon.DistributedToon(base.cr, bFake=True)
            self.giftAvatar.doId = self.frienddoId
            self.giftAvatar.forceAllowDelayDelete()
            self.giftAvatar.generate()
            base.cr.getAvatarDetailsExtended(self.giftAvatar, self.__handleAvatarDetails, 'DistributedToon')
            self.gotAvatar = 0
