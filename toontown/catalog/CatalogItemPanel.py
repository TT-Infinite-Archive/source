...
class CatalogItemPanel(DirectFrame):
    ...
    def load(self):
        ...
        if typeCode == CatalogItemTypes.PET_TRICK_ITEM and not hasattr(base.localAvatar, 'petDNA'):
            return False
    ...
    def updateGiftButton(self, giftUpdate = 0):
        ...
        if numFriends > 0:
            ...
            if self['item'].isGift() <= 0:
                ...
                if self['item'].forBoysOnly() and avatar.getStyle().getGender() == 'f' or \
                                self['item'].forGirlsOnly() and avatar.getStyle().getGender() == 'm':
                    self.giftButton.show()
                    self.giftButton['state'] = DGG.DISABLED
