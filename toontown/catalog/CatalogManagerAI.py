...
class CatalogManagerAI(DistributedObjectAI.DistributedObjectAI):
    ...
    skipWeeks = ConfigVariableBool('catalog-skip-weeks', False).getValue()
    ...
    def deliverCatalogFor(self, avatar):
        ...
        self.notify.debug(f"Avatar {avatar.doId} at catalog week {currentWeek} (previous {previousWeek}).")
        ...
        if newMonthlyCatalog or newWeeklyCatalog:
            ...
            self.air.writeServerEvent(
                'issue-catalog', avatar.doId, f"{modCurrentWeek}")
    ...
    def purchaseItem(self, avatar, item, optional):
        ...
        if deliveryTime == 0:
            ...
            self.notify.debug(f"Avatar {avatar.doId} purchased {item}, delivered immediately.")
    ...
    def deductMoney(self, avatar, price, item):
        ...
        self.air.writeServerEvent(
            'catalog-purchase', avatar.doId, f"{price}|{item}"
        )
        #pdb.set_trace()
    ...
    def setDelivery(self, avatar, item, deliveryTime, retcode, doUpdateLater):
        if len(avatar.mailboxContents) + len(avatar.onOrder) >= ToontownGlobals.MaxMailboxContents:
            self.notify.debug(f"Avatar {avatar.doId} has {len(avatar.mailboxContents)} in mailbox and {len(avatar.onOrder)} on order, too many.")
            ...
            self.notify.debug(f"Avatar {avatar.doId} purchased {item}, to be delivered later.")
            now = (int)(time.time() / 60 + 0.5)
            #if avatar.instantDelivery:
            #    self.notify.debug(f"Avatar {avatar.doId} has instant delivery enabled, delivering {item} now!")
            #    item.deliveryDate = int(now)
            #else:
    ...
    def startCatalog(self):
        ...
        if avatar and avatar.catalogScheduleNextTime == 0:
            self.notify.debug(f"starting catalog for {avatar.getName()}")
            self.deliverCatalogFor(avatar)
