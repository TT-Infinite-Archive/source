...
class SuitPlannerInteriorAI:
    ...
    def __genLevelList(self, bldgLevel, currFloor, numFloors):
        ...
        hoodId = ZoneUtil.getCanonicalSafeZoneId(self.zoneId)
        zoneMin, zoneMax = SuitBuildingGlobals.zone2MinMaxLevel.get(hoodId, (0, 0))
        ...
        if currFloor + 1 == numFloors:
            ...
            if newLvl < lvlMax and needsAdjustment:
                newLvl = min(random.choice((lvlMax + 1, lvlMax + 2)), 12)
            elif newLvl > lvlMax and needsAdjustment:
                newLvl = lvlMax
