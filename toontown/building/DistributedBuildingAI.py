...
from toontown.cogdominium.CogdoGameConsts import *
...
class DistributedBuildingAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def enterWaitForVictorsFromCogdo(self, victorList, savedBy):
        for t in victorList:
            ...
            event = self.air.getAvatarExitEvent(t)
            self.accept(event, self.setVictorExited, extraArgs=[t])

        self.b_setVictorList(victorList)
        self.updateSavedBy(savedBy)
        self.victorResponses = [0] * len(victorList)
    ...
    def enterCogdo(self):
        ...
        # HOTFIX: Remove this hack-patch once the respective games have been completed.
        if self.track == 'm':
            #  This is a Cashbot building. It's a Lawbot game.
            self.track = 'l'
        elif self.track == 'c':
            # This is a Bossbot building. It's a Sellbot game.
            self.track = 's'
