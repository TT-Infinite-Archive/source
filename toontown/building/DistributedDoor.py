...
class DistributedDoor(DistributedObject.DistributedObject, DelayDeletable):
    def __init__(self, cr):
        ...
        self.openSfx = loader.loadSfx('phase_3.5/audio/sfx/Door_Open_1.ogg')
        self.closeSfx = loader.loadSfx('phase_3.5/audio/sfx/Door_Close_1.ogg')
        self.nametag = None
    ...
    def hideDoorParts(self):
        if self.doorType in self.specialDoorTypes or self.doorType in (DoorTypes.EXT_STANDARD,):
    ...
    def avatarEnqueueTrack(self, avatar, duration):
        ...
        if avatar.isGoofy == 1:
            avatar.goofy.loop('walk')
            # Make sure Goofy's animation is not on reverse when entering door.
            avatar.goofy.setPlayRate(1, 'walk')
    ...
    def getAnimStateInterval(self, avatar, animName):
        ...
        elif avatar.isGoofy == 1:
            return Func(avatar.goofy.loop, animName)
        else:
                ...
                if base.wantDoorInteract:
                    if not hasattr(self, "enterText"):
                        ...
                        if ZoneUtil.isInterior(self.zoneId):
                            ...
                            state = "enter"
                        if name != '':
                            text = ("Press " + str(base.INTERACT_KEY).upper() + " to %s %s" % (state, name))
                        else:
