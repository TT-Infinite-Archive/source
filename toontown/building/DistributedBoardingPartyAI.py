...
from otp.distributed.OtpDoGlobals import OTP_DO_ID_GLOBAL_GROUP_TRACKER
...
from toontown.building.GroupTrackerGlobals import ZONE_TO_CATEGORY_LIST, CATEGORY_NO_UPDATE, GROUP_TYPE_NORMAL
GROUPMEMBER = 0
GROUPINVITE = 1
...
class DistributedBoardingPartyAI(DistributedObjectAI.DistributedObjectAI, BoardingPartyBase.BoardingPartyBase):
    ...
    def generate(self):
        ...
        for elevatorId in self.elevatorIdList:
            ...
            if elevator is None:
                self.notify.warning('Tried to generate boarding party for non existent elevator %d' % elevatorId)
                self.delete()
                return
    ...
    def removeWatchAvStatus(self, avId):
        ...
        self.ignore(messageToonAdded)
        self.ignore(messageToonReleased)
    ...
    def requestInvite(self, inviteeId):
        ...
        self.notify.debug('Avatar %d requesting to invite %d to a Boarding Group' % (inviterId, inviteeId))

        invitee = simbase.air.doId2do.get(inviteeId)
        originalInviteeId = inviteeId
        merger = False
        if invitee and invitee.battleId != 0:
            # Deny invite, player in battle
        ...
        if self.hasPendingInvite(inviteeId):
            # Deny invite, already being invited
            reason = BoardingPartyBase.BOARDCODE_PENDING_INVITE
            self.sendUpdateToAvatarId(inviteeId, 'postInviteNotQualify', [inviteeId, reason, 0])
            self.sendUpdateToAvatarId(inviteeId, 'postMessageInvitationFailed', [inviterId])
            return
        if self.__isInElevator(inviteeId):
            # Deny invite, too busy
            reason = BoardingPartyBase.BOARDCODE_IN_ELEVATOR
            self.sendUpdateToAvatarId(inviterId, 'postInviteNotQualify', [inviteeId, reason, 0])
            self.sendUpdateToAvatarId(inviteeId, 'postMessageInvitationFailed', [inviterId])
            return
        if self.hasActiveGroup(inviteeId):
            # This player is already in a group
            if self.hasActiveGroup(inviterId):
                # Our inviter is also in a group, check for possible merge
                targetLeaderId = self.avIdDict[inviteeId]
                leaderId = self.avIdDict[inviterId]

                if self.hasPendingInvite(targetLeaderId):
                    # Target's leader already being invited to a marge, deny
                    reason = BoardingPartyBase.BOARDCODE_PENDING_INVITE
                    self.sendUpdateToAvatarId(inviterId, 'postInviteNotQualify', [inviteeId, reason, 0])
                    self.sendUpdateToAvatarId(inviteeId, 'postMessageInvitationFailed', [inviterId])
                    return

                if len(self.getGroupMemberList(leaderId)) + len(self.getGroupMemberList(targetLeaderId)) > self.maxSize:
                    # This group will be too large with a merge, deny
                    reason = BoardingPartyBase.BOARDCODE_GROUPS_TO_LARGE
                    self.sendUpdateToAvatarId(inviterId, 'postInviteNotQualify', [inviteeId, reason, 0])
                    self.sendUpdateToAvatarId(inviteeId, 'postMessageInvitationFailed', [inviterId])
                    return

                # This group can be merged, send invite to leader
                invitee = simbase.air.doId2do.get(targetLeaderId)
                inviteeId = targetLeaderId
                merger = True
            else:
                # Player is already in another group, deny
    ...
    def requestAcceptInvite(self, leaderId, inviterId):
        ...
        if inviteeId in self.avIdDict:
            if inviteeId in self.mergeDict:
                ...
                # Let the group tracker know the other group is rip (this also updates)
    ...
    def removeFromGroup(self, leaderId, memberId, kick = 0, post = 1):
        ...
        if memberId == leaderId or len(group[0]) < 2:
            ...
            # Let Group Tracker know this group was removed.
            self.removeBroadcast(leaderId)
    ...
    def broadcastGroup(self, leaderId):
        ...
        if leaderId in self.groupListDict:
            ...
            if av:
                ...
                shardId = self.air.districtId
                category = ZONE_TO_CATEGORY_LIST[self.zoneId][0]
                self.air.globalGroupTracker.addGroupAI(leaderId, leaderName, shardName, shardId, category, memberIds,
                                                       memberNames, show, GROUP_TYPE_NORMAL, self.zoneId)
                self.air.addPostRemove(self.__makePostRemoveMsg(leaderId))

    def __makePostRemoveMsg(self, leaderId):
        field = self.air.dclassesByName['GlobalGroupTrackerAI'].getFieldByName('updateGroup')
        return field.aiFormatUpdate(OTP_DO_ID_GLOBAL_GROUP_TRACKER, OTP_DO_ID_GLOBAL_GROUP_TRACKER,
