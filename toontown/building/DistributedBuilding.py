from panda3d.core import ConfigVariableBool, DecalEffect, DepthOffsetAttrib, NodePath, Point3, TextNode, VBase4, Vec3
...
FO_DICT = {'s': 'tt_m_ara_cbe_fieldOfficeMoverShaker',
 ...
 'm': 'tt_m_ara_cbe_fieldOfficeLegalEagle', # Cashbot
 'c': 'tt_m_ara_cbe_fieldOfficeMoverShaker'} # Bossbot
...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def enterBecomingToon(self, ts):
        if hasattr(self.cr.playGame, 'dnaStore'):
            if self.block in self.cr.playGame.dnaStore.suitBlocks:
                del self.cr.playGame.dnaStore.suitBlocks[self.block]
    ...
    def setupSuitBuilding(self, nodePath):
        ...
        suitNP = dnaStore.findNode('suit_landmark_' + chr(self.track) + '1')
