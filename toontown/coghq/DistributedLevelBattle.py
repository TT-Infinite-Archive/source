...
class DistributedLevelBattle(DistributedBattle.DistributedBattle):
    ...
    def setBattleCellId(self, battleCellId):
        ...
        def doPlacement(levelList, self = self):
            ...
            entity = self.level.getEntity(spec['parentEntId'])
            entityPos = entity.getPos(render)
            entityHpr = entity.getHpr(render)
            self.notify.debug('Entity pos: ' + str(entityPos))
            self.notify.debug('Entity hpr: ' + str(entityHpr))
            self.d_setPosition(entityPos[0], entityPos[1], entityPos[2], entityHpr[0], entityHpr[1], entityHpr[2])
    ...
    def d_setPosition(self, x, y, z, h, p, r):
        self.sendUpdate('setPosition', [x, y, z, h, p, r])
