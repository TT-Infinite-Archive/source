...
class DistributedCashbotBossCrane(DistributedObject.DistributedObject, FSM.FSM):
    ...
    def setupCable(self):
        ...
        if self.index in (4, 5):
            self.handler.setStaticFrictionCoef(0.05)
        else:
        ...
        if self.index in (4, 5):
            magnetModel = self.boss.magnetB.copyTo(self.wiggleMagnet)
            magnetModel.setScale(1.25)
            self.armMinH = -55
            self.armMaxH = 55
        else:
    ...
    def grabObject(self, obj):
        ...
        self.heldObject = obj
        if self.index in (4, 5):
            self.handler.setDynamicFrictionCoef(obj.craneFrictionCoef * .65)
            self.slideSpeed = obj.craneSlideSpeed * .65
            self.rotateSpeed = obj.craneRotateSpeed * .65
        else:
    ...
    def setIndex(self, index):
        self.index = index
        if index in (4, 5):
            self.emptySlideSpeed *= .65
            self.emptyRotateSpeed *= .65
            self.emptyFrictionCoef *= .65
            self.rotateSpeed = self.emptyRotateSpeed
            self.slideSpeed = self.emptySlideSpeed
