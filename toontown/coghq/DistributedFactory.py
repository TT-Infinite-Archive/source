...
class DistributedFactory(DistributedLevel.DistributedLevel, FactoryBase.FactoryBase):
    ...
    def generate(self):
        ...
        self.meritCounter = FactoryMeritCounter(self, TreasureS, FactoryGlobals.MaxMerits, parent=base.a2dLeftCenter, pos=(0.4, 0.0, 0.6))
        self.meritCounter.load()
        self.questPoster = FactoryQuestPoster(self, parent=base.a2dLeftCenter, pos=(0.3, 0.0, 0.3))
        base.setCellsActive([base.leftCells[0]], False)
        if __dev__:
    ...
    def delete(self):
        ...
        if hasattr(self, 'meritCounter'):
            if self.meritCounter is not None:
                self.meritCounter.destroy()
                self.meritCounter = None
        if hasattr(self, 'questPoster'):
            if self.questPoster is not None:
                self.questPoster.destroy()
                self.questPoster = None
        self.ignore('SOSPanelEnter')
        base.setCellsActive([base.leftCells[0]], True)
        if __dev__:
    ...
    def setMeritCount(self, amount):
        self.currentMeritCount = amount
        self.updateMeritCounter(self.currentMeritCount)

    def updateMeritCounter(self, amount):
        if self.meritCounter is not None:
            self.meritCounter.setMeritBar(amount)

    def setQuestPoster(self, questId):
        self.questPoster.load()
        self.questPoster.setQuest(questId)
