...
class ToontownClientRepository(OTPClientRepository.OTPClientRepository):
    ...
    def enterChooseAvatar(self, avList):
        ...
        if (self.music is None) and base.musicManagerIsValid:
            self.music = base.musicManager.getSound('phase_3/audio/bgm/tti_classic_theme.ogg')
    ...
    def getAvatarDetailsExtended(self, avatar, func, *args):
        pad = ScratchPad()
        pad.func = func
        pad.args = args
        pad.avatar = avatar
        pad.delayDelete = DelayDelete.DelayDelete(avatar, 'getAvatarDetails')
        self.__queryAvatarMap[avatar.doId] = pad
        self.__sendGetAvatarDetails(avatar.doId, pet=(args[0].endswith("Pet")), extended=True)
    ...
    def __sendGetAvatarDetails(self, avId, pet=0, extended=False):
        if extended:
            self.ttiFriendsManager.d_getAvatarDetailsExtended(avId)
        elif pet:
            self.ttiFriendsManager.d_getPetDetails(avId)
        else:    
            self.ttiFriendsManager.d_getAvatarDetails(avId)

    def n_handleGetAvatarDetailsResp(self, avId, fields, extended=False):
    ...
    def sendGetIgnoreListRequest(self):
        self.ttiFriendsManager.d_requestIgnoreList()
    ...
    def handleGetFriendsList(self, resp):
        for toon in resp:
            ...
            if doId in self.friendPendingChatSettings:
                ...
                handle.setCommonAndWhitelistChatFlags(*self.friendPendingChatSettings[doId][0])
                handle.setChatMode(self.friendPendingChatSettings[doId][1])
    ...
    def handleFriendOnline(self, doId, commonChatFlags, whitelistChatFlags, chatMode):
        self.notify.debug('Friend %d now online. common=%d whitelist=%d chatMode=%s' % (doId, commonChatFlags, whitelistChatFlags, chatMode))
        if doId not in self.friendsOnline:
            ...
            if not self.friendsOnline[doId]:
                self.friendPendingChatSettings[doId] = ((commonChatFlags, whitelistChatFlags), chatMode)
            elif hasattr(self.friendsOnline[doId], 'setChatMode'):
                self.friendsOnline[doId].setChatMode(chatMode)
