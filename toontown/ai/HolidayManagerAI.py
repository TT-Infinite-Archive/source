...
class HolidayManagerAI:
    ...
    def startHoliday(self, holidayId):
        if holidayId == TRICK_OR_TREAT:
            ...
            #for hood in self.air.hoods:
                #if hood.zoneId == 5000: # Daisy's Gardens
                    #hood.startupGreenToonManager()
        ...
        elif holidayId == DOUBLE_PROGRESSION_HOLIDAY:
            self.xpMultiplier *= 2
            self.meritMultiplier *= 2
            self.rewardMultiplier *= 2
            self.air.questManager.progressMultiplier *= 2

        elif holidayId == MORE_XP_HOLIDAY:
            self.xpMultiplier *= 2
    ...
    def yearlyHolidayTask(self, task=None):
        for holiday in DEFAULT_YEARLY_HOLIDAYS:
            ...
            if endDate.month < startDate.month:
                endDate = endDate.replace(year=now.year + 1)
