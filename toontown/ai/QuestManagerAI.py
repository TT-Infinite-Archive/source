...
class QuestManagerAI:
                ...
                if npc.npcId == toNpcId:
                    ...
                    for i in xrange(0, len(avQuests), 5):
                        ...
                        if questDesc[QuestIdIndex] == questId:
                            questDesc[QuestProgressIndex] += progress * self.progressMultiplier
                ...
                if questClass.getHolder() == Quests.AnyFish:
                    if not questClass.getCompletionStatus(av, questDesc) == Quests.COMPLETE:
                        ...
                        if chance >= baseChance:
                            questDesc[QuestProgressIndex] += 1 * self.progressMultiplier
        ...
        for i in xrange(0, len(avQuests), 5):
            ...
            if questClass.getCompletionStatus(av, questDesc) == Quests.INCOMPLETE:
                ...
                if isinstance(questClass, Quests.RecoverItemQuest):
                    ...
                    # Are we in the correct area to recover the items?
                    if not questClass.isLocationMatch(taskZoneId):
                        questList.append(questDesc)
                        continue
                    ...
                    for suit in suitsKilled:
                        ...
                        # Because the RecoverItemQuest class doesn't have a
                        ...
                        if (questClass.getHolder() == Quests.Any) or \
                                (questClass.getHolderType() == 'type' and questClass.getHolder() == suit['type']) or \
                                (questClass.getHolderType() == 'track' and questClass.getHolder() == suit['track']) or \
                                (questClass.getHolderType() == 'level' and questClass.getHolder() <= suit['level']):
    ...
    def toonKilledBuilding(self, av, type, difficulty, floors, zoneId, activeToons):
        ...
        for i in xrange(0, len(avQuests), 5):
            ...
            questClass = Quests.getQuest(questDesc[QuestIdIndex])
            if questClass.getCompletionStatus(av, questDesc) == Quests.INCOMPLETE:
                if isinstance(questClass, Quests.BuildingQuest) or isinstance(questClass, Quests.BuildingNewbieQuest):
