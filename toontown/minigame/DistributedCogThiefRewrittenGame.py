...
class DistributedCogThiefRewrittenGame(DistributedMinigame):
    ...
    def __init__(self, cr):
        ...
        self.gameFSM = ClassicFSM.ClassicFSM('DistributedCogThiefRewrittenGame', [State.State('off', self.enterOff, self.exitOff, ['play']), State.State('play', self.enterPlay, self.exitPlay, ['cleanup']), State.State('cleanup', self.enterCleanup, self.exitCleanup, [])], 'off', 'cleanup')
    ...
    def getTitle(self):
        return TTLocalizer.CogThiefRewrittenGameTitle
    ...
    def loadCogs(self):
        ...
        for suitIndex in xrange(self.getNumCogs()):
            ...
            suit = CogThiefRewritten.CogThiefRewritten(suitIndex, st, self, self.getCogSpeed())
