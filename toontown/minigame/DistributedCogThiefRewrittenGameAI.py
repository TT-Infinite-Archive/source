...
class DistributedCogThiefRewrittenGameAI(DistributedMinigameAI.DistributedMinigameAI):
    notify = directNotify.newCategory('DistributedCogThiefRewrittenGameAI')
    ExplodeWaitTime = 6.0 + CTGG.LyingDownDuration

    def __init__(self, air, minigameId):
        try:
            self.DistributedCogThiefRewrittenGameAI_initialized
        except:
            self.DistributedCogThiefRewrittenGameAI_initialized = 1
            DistributedMinigameAI.DistributedMinigameAI.__init__(self, air, minigameId)
            self.gameFSM = ClassicFSM.ClassicFSM('DistributedCogThiefRewrittenGameAI', [State.State('inactive', self.enterInactive, self.exitInactive, ['play']), State.State('play', self.enterPlay, self.exitPlay, ['cleanup']), State.State('cleanup', self.enterCleanup, self.exitCleanup, ['inactive'])], 'inactive', 'inactive')
    ...
    def initCogInfo(self):
        for cogIndex in xrange(self.getNumCogs()):
            self.cogInfo[cogIndex] = {'pos': Point3(CogThiefRewrittenGameGlobals.CogStartingPositions[cogIndex]),
             'goal': CTGG.NoGoal,
             'goalId': CTGG.InvalidGoalId,
             'barrel': CTGG.NoBarrelCarried}

    def initBarrelInfo(self):
        for barrelIndex in xrange(CogThiefRewrittenGameGlobals.NumBarrels):
            self.barrelInfo[barrelIndex] = {'pos': Point3(CogThiefRewrittenGameGlobals.BarrelStartingPositions[barrelIndex]),
    ...
    def throwingPie(self, todo0, todo1, todo2, todo3, todo4, todo5):
        #throwingPie(uint32, int32, int32/100, int32/100, int32/100, int32/100) clsend broadcast;  // field 2222
        return

    def updateSuitGoal(self, todo0, todo1, todo2, todo3, todo4, todo5, todo6, todo7):
        #updateSuitGoal(int32, int32, int8, int8, int64, int32/100, int32/100, int32/100) broadcast;  // field 2227
        return