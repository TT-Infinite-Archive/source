...
class LocalAvatar(DistributedAvatar.DistributedAvatar, DistributedSmoothNode.DistributedSmoothNode):
    ...
    def __init__(self, cr, chatMgr, talkAssistant = None, passMessagesThrough = False):
        ...
        self.accept('onFloor', self.handleOnFloor)
        self.accept('offFloor', self.handleOffFloor)
        ...
        self.rewardTextQueue = []
        self.rewardTextLines = []
        self.rewardTextFadeInTrack = None
    ...
    def initializeCollisions(self):
        ...
        self.avatarFloorCollisionBroadcaster.setInPattern('onFloor')
        self.avatarFloorCollisionBroadcaster.setOutPattern('offFloor')
        self.avatarFloorCollisionBroadcaster.addCollider(
            self.physControls.cRayNodePath, self.cRayMoveNodePath)
        self.cTrav.addCollider(self.physControls.cRayNodePath,
                               self.avatarFloorCollisionBroadcaster)
    ...
    def displayRewardText(self, rewardText, enqueue=True):
        # Do we need to delay the appearance of this reward text so that we can
        # let the fade in animation of another reward text finish? If so,
        # enqueue it:
        if enqueue and (self.rewardTextFadeInTrack is not None):
            if self.rewardTextFadeInTrack.isPlaying() or self.rewardTextQueue:
                self.rewardTextQueue.append(rewardText)
                if not self.rewardTextFadeInTrack.isPlaying():
                    self.displayNextRewardText()
                return

        # If this reward text was queued, dequeue it:
        if (not enqueue) and (rewardText in self.rewardTextQueue):
            self.rewardTextQueue.remove(rewardText)

        # Make room for our new reward text if necessary:
        for rewardTextLine in self.rewardTextLines:
            lineCount = len(self.rewardTextLines)
            lineNumber = lineCount - self.rewardTextLines.index(rewardTextLine)
            LerpPosInterval(
                rewardTextLine, 0.75, Point3(-0.525, 0, 0.1 * (lineNumber + 1)),
                Point3(-0.525, 0, 0.1 * lineNumber), blendType='easeOut'
            ).start()
        ...
        # Animate it:
        self.rewardTextFadeInTrack = Sequence(
            Wait(0.5),
            Parallel(
                LerpColorScaleInterval(
                    rewardTextLine, 1, Vec4(1, 1, 0, 1), Vec4(1, 1, 0, 0),
                    blendType='easeInOut'),
                LerpPosInterval(
                    rewardTextLine, 1, Point3(-0.525, 0, 0.1),
                    Point3(-0.525, 0, 0.075), blendType='easeInOut')
            ),
            Func(self.displayNextRewardText)
        )
        self.rewardTextFadeInTrack.start()

        # Handle the cleanup process after a short amount of time:
        Sequence(
            Wait(5),
            LerpColorScaleInterval(
                rewardTextLine, 1, Vec4(1, 1, 0, 0), Vec4(1, 1, 0, 1),
                blendType='easeOut'),
            Func(rewardTextLine.destroy),
            Func(self.rewardTextLines.remove, rewardTextLine)
        ).start()
...
@magicWord(category=CATEGORY_USER, types=[float, float, float])
def xyz(x, y, z):
    """
