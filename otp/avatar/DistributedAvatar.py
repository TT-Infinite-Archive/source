    ...
    def showHpString(self, text, duration=0.85, scale=0.7, r=1.0, g=0.0, b=0.0):
        if self.HpTextEnabled and not self.ghostMode:
            if text != '':
                ...
                self.HpTextGenerator.clearShadow()
                self.HpTextGenerator.setAlign(TextNode.ACenter)
                self.HpTextGenerator.setTextColor(r, g, b, 1.0)
                self.hpTextNode = self.HpTextGenerator.generate()
                self.hpText = self.attachNewNode(self.hpTextNode)
                self.hpText.setScale(scale)
...
@magicWord(category=CATEGORY_MODERATOR, types=[str, int, str])
...
@magicWord(category=CATEGORY_MODERATOR, types=[str, int, int, str])
